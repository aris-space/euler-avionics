...
import sys
from myutils import fontStyle_large, fontStyle_medium, borderwidth
...
class ConnectionWindow(Frame):
    def __init__(self, parent: Tk, main):
        ...
        self._root.geometry('%dx%d+%d+%d' % (w*4.5, h*4.5, x, y))
    ...
    def __setup__(self):
        frame1 = LabelFrame(self._root, text='Settings', font=fontStyle_large, borderwidth=borderwidth)
        frame2 = LabelFrame(self._root, text='Available ports', font=fontStyle_large, borderwidth=borderwidth)

        label_available_ports = Label(frame2, text=available_ports())

        label_port = Label(frame1, text='Port', font=fontStyle_medium)
        label_baud = Label(frame1, text='Baud rate', font=fontStyle_medium)

        self.entry1 = Entry(frame1, font=fontStyle_medium)
        self.entry2 = Entry(frame1, font=fontStyle_medium)

        button_connect = Button(frame1, text='Connect', font=fontStyle_medium, command=self.connect_xbee)
