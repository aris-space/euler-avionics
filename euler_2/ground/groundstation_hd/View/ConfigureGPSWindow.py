...
class ConfigureGPSWindow(Frame):
    def __init__(self, parent: Tk, main):
        ...
        self._root.geometry('%dx%d+%d+%d' % (2*w, 2*h, x, y))
    ...
    def __setup__(self):
        frame_timezone = LabelFrame(self._root, text='Timezone', font=fontStyle_large, borderwidth=borderwidth)
        frame_timezone.pack(fill='both', padx=20)

        label1 = Label(frame_timezone, text='UTC+')
        label1.grid(row=0, column=0, padx=(5, 0), pady=5)
        self.entry1 = Entry(frame_timezone, width=10)
        self.entry1.grid(row=0, column=1, padx=(0, 5), pady=10)
        self.entry1.delete(0, 'end')
        self.entry1.insert(0, str(self._main.time_offset))

        frame_orientation = LabelFrame(self._root, text='Orientation', font=fontStyle_large, borderwidth=borderwidth)
        frame_orientation.pack(fill='both', padx=20)
