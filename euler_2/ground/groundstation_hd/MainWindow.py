...
from myutils import sb_names, gps_names, battery_names, fsm_names, hardware_names, dict_command_msg, button_text, \
    data_struct, fontStyle_large, fontStyle_medium, fontStyle_small, fontStyle_menu, borderwidth
import math
...
class MainWindow(Frame):
    ...
    def __setup__(self):
        ...
        self.file_menu.add_command(label="New", command=self.donothing, font=fontStyle_menu)
        self.file_menu.add_command(label="Open", command=self.donothing, font=fontStyle_menu)
        self.file_menu.add_command(label="Save", command=self.donothing, font=fontStyle_menu)
        self.file_menu.add_command(label="Save as...", command=self.donothing, font=fontStyle_menu)
        self.file_menu.add_separator()
        self.file_menu.add_command(label="Exit", command=self._root.quit, font=fontStyle_menu)

        self.connection_menu.add_command(label="Serial connection", command=self.client_connect_serial, font=fontStyle_menu)
        self.connection_menu.add_command(label="Start reception", command=self.start_reception, font=fontStyle_menu)
        self.connection_menu.add_command(label="Stop reception", command=self.stop_reception, font=fontStyle_menu)

        self.plot_menu.add_command(label="Start live plot", command=self.start_plot, font=fontStyle_menu)
        self.plot_menu.add_command(label="Stop live plot", command=self.stop_plot, font=fontStyle_menu)

        self.settings_menu.add_command(label="Commands", command=self.command_settings, font=fontStyle_menu)
        self.settings_menu.add_command(label='Print raw data (on/off)', command=self.toggle_verbose, font=fontStyle_menu)
        self.settings_menu.add_command(label='GPS', command=self.client_configure_gps, font=fontStyle_menu)
        self.settings_menu.add_command(label='Plot', command=self.client_configure_plot, font=fontStyle_menu)

        self.help_menu.add_command(label="About", command=self.about_window, font=fontStyle_menu)
        # self.help_menu.add_command(label="Manual", command=self.manual_window)

        # ==============================================================================================================
        # create subframes
        # ==============================================================================================================
        self.frame_upper = tk.Frame(self._root, width=500, height=500)
        self.frame_lower = tk.Frame(self._root, width=150, height=100)
        self.frame_upper_left = tk.Frame(self.frame_upper,
                                         width=10,
                                         height=50)
        self.frame_upper_middle = tk.LabelFrame(self.frame_upper,
                                                text='Velocity [m/s]',
                                                font=fontStyle_large,
                                                borderwidth=borderwidth,
                                                width=500,
                                                height=500)
        self.frame_upper_right = tk.LabelFrame(self.frame_upper,
                                               text='Altitude [m]',
                                               font=fontStyle_large,
                                               borderwidth=borderwidth,
                                               width=500,
        ...
        # ==============================================================================================================
        self.frame_recording = tk.LabelFrame(self.frame_upper_left,
                                             text='Recording',
                                             font=fontStyle_large,
                                             borderwidth=borderwidth,
                                             width=80, height=10)
        self.label_file_name = tk.Label(self.frame_recording,
                                        text='file name',
                                        font=fontStyle_small)
        self.button_start_rec = tk.Button(self.frame_recording,
                                          text='Start',
                                          font=fontStyle_medium,
                                          command=self.start_recording)
        self.button_stop_rec = tk.Button(self.frame_recording,
                                         text='Stop',
                                         font=fontStyle_medium,
                                         command=self.stop_recording)
        self.entry_name = tk.Entry(self.frame_recording, font=fontStyle_small)

        # ==============================================================================================================
        # add frame for commands
        # ==============================================================================================================
        self.frame_command = tk.LabelFrame(self.frame_upper_left,
                                           text='Commands',
                                           font=fontStyle_large,
                                           borderwidth=borderwidth,
                                           width=80, height=10)
        ...
        # add log frame
        # ==============================================================================================================
        self.frame_log = tk.LabelFrame(self.frame_upper_left,
                                       text='Log',
                                       font=fontStyle_large,
                                       borderwidth=borderwidth,
                                       width=20, height=10)
        ...
        # ==============================================================================================================
        self.frame_sb = tk.LabelFrame(self.frame_lower,
                                      text='Sensorboard data',
                                      font=fontStyle_large,
                                      borderwidth=borderwidth,
                                      width=250, height=75)
        ...
        for i in range(len(sb_names)):
            ...
            self.label_sb_name.append(tk.Label(self.frame_sb, text=sb_names[i], font=fontStyle_medium, ))
        ...
        # add GPS frame
        # ==============================================================================================================
        self.frame_gps = tk.LabelFrame(self.frame_lower,
                                       text='GPS',
                                       font=fontStyle_large,
                                       borderwidth=borderwidth,
                                       width=200, height=75)
        ...
        for i in range(len(gps_names)):
            ...
            self.label_gps_name.append(tk.Label(self.frame_gps, text=gps_names[i], font=fontStyle_medium, ))

            self.label_gps_name[i].grid(row=i, column=0, sticky='W', padx=(30, 10))
            self.label_gps_val[i].grid(row=i, column=1, padx=(10, 30))

        # ==============================================================================================================
        # add Hardware frame
        # ==============================================================================================================
        self.frame_hardware = tk.LabelFrame(self.frame_lower,
                                            text='Hardware',
                                            font=fontStyle_large,
                                            borderwidth=borderwidth,
                                            width=100, height=75)
        ...
        for i in range(len(hardware_names)):
            ...
            self.label_hardware_name.append(
                tk.Label(self.frame_hardware, text=hardware_names[i], font=fontStyle_medium))

            if i == 0:
                self.label_hardware_name[i].grid(row=i, column=0, sticky='W', padx=(30, 10), pady=(10, 0))
                self.label_hardware_val[i].grid(row=i, column=1, padx=(10, 30), pady=(10, 0))
            else:
                self.label_hardware_name[i].grid(row=i, column=0, sticky='W', padx=(30, 10))
                self.label_hardware_val[i].grid(row=i, column=1, padx=(10, 30))
        # ==============================================================================================================
        # add battery monitoring frame
        # ==============================================================================================================
        self.frame_battery = tk.LabelFrame(self.frame_lower,
                                           text='Power',
                                           font=fontStyle_large,
                                           borderwidth=borderwidth,
                                           width=100, height=75)
        ...
        for i in range(len(battery_names)):
            ...
            self.label_battery_names.append(tk.Label(self.frame_battery, text=battery_names[i], font=fontStyle_medium))

            if i == 0:
                self.label_battery_names[i].grid(row=i, column=0, padx=(30, 10), pady=(10, 0), sticky='W')
                self.label_battery_val[i].grid(row=i, column=1, padx=(10, 30), pady=(10, 0))
            else:
                self.label_battery_names[i].grid(row=i, column=0, padx=(30, 10), sticky='W')
                self.label_battery_val[i].grid(row=i, column=1, padx=(10, 30))

        # ==============================================================================================================
        # add FSM frame
        # ==============================================================================================================
        self.frame_fsm = tk.LabelFrame(self.frame_lower,
                                       text='FSM data',
                                       font=fontStyle_large,
                                       borderwidth=borderwidth,
                                       width=200, height=75)
        ...
        for i in range(len(fsm_names)):
            ...
            self.label_fsm_names.append(tk.Label(self.frame_fsm, text=fsm_names[i], font=fontStyle_medium))

            if i == 0:
                self.label_fsm_names[i].grid(row=i, column=0, padx=(30, 10), sticky='W', pady=(10, 0))
                self.label_fsm_val[i].grid(row=i, column=1, padx=(10, 30), pady=(10, 0))
            else:
                self.label_fsm_names[i].grid(row=i, column=0, padx=(30, 10), sticky='W')
                self.label_fsm_val[i].grid(row=i, column=1, padx=(10, 30))
        ...
        self.frame_lower.pack(side="top", fill='x', expand=False)

        self.frame_upper_left.pack(side='left', fill='y', expand=False, pady=(10, 10), padx=20)
        ...
        self.frame_recording.pack(side='top', fill='x', expand=False)
        self.frame_command.pack(side='top', fill='x', expand=False)
        self.frame_log.pack(side='top', fill='both', expand=True)

        st = scrolledtext.ScrolledText(self.frame_log, state='disabled', width=20, height=5)
        # st.configure(font='TkFixedFont')
        st.configure(font=fontStyle_small)
        ...
        for i in range(len(button_text)):
            if i < half:
                self.buttons[i].grid(row=0, column=i, padx=8, pady=8)
            else:
                self.buttons[i].grid(row=1, column=i - half, padx=8, pady=8)

        self.logger.addHandler(LoggingHandler(st))

        self.frame_sb.pack(side="left", fill="both", expand=True, padx=20, pady=20)
        # self.sep2.pack(side="left", fill="both")
        self.frame_gps.pack(side="left", fill="both", expand=True, padx=(0, 20), pady=20, ipady=10)
        self.frame_hardware.pack(side="left", fill="both", expand=True, padx=(0, 20), pady=20)
        self.frame_battery.pack(side="left", fill="both", expand=True, padx=(0, 20), pady=20)
        self.frame_fsm.pack(side="left", fill="both", expand=True, padx=(0, 20), pady=20)

        self.label_file_name.grid(row=0, column=0, pady=20, padx=20)
        self.entry_name.grid(row=0, column=1, pady=20)
        self.button_start_rec.grid(row=0, column=2, padx=(30, 10), pady=10)
        self.button_stop_rec.grid(row=0, column=3, padx=10, pady=10)
        ...
        self.label_sb_val[3].grid(row=0, column=4, padx=(10, 30), pady=(10, 0))
        self.label_sb_val[4].grid(row=1, column=4, padx=(10, 30))
        self.label_sb_val[5].grid(row=2, column=4, padx=(10, 30))
        self.label_sb_val[6].grid(row=3, column=4, padx=(10, 30))
        self.label_sb_val[7].grid(row=4, column=4, padx=(10, 30))
        self.label_sb_val[8].grid(row=5, column=4, padx=(10, 30))
