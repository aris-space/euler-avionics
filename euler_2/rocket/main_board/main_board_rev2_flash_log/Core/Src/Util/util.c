...
osStatus_t logFlash(void *flash_data) {
	flash_data_t temp_data = { 0 };
	memcpy(&temp_data, flash_data, sizeof(temp_data));
	return osMessageQueuePut(logf_queue, &temp_data, 0U, 0U);
}

osStatus_t logSensorF(timestamp_t ts, board_id_t sensor_board_id,
		sensor_type_e sensor_type, void *sensor_data) {
	flash_log_elem_t log_elem = { .log_type = SENSOR };
	uint16_t data_size = 0;
	switch (sensor_type) {
	case BARO:
		data_size = sizeof(baro_data_t);
		break;
	case IMU:
		data_size = sizeof(imu_data_t);
		break;
	case BATTERY:
		data_size = sizeof(battery_data_t);
		break;
	}
	log_elem.u.sensor_log.sens_type = sensor_type;
	log_elem.u.sensor_log.sensor_board_id = sensor_board_id;
	log_elem.u.sensor_log.ts = ts;
	memcpy(&log_elem.u.sensor_log.sensor_data, sensor_data, data_size);
	return osMessageQueuePut(logf_queue, &log_elem, 0U, 0U);
}

osStatus_t logMsgF(timestamp_t ts, char *msg) {
	flash_log_elem_t log_elem = { .log_type = MSG };
	/* sizeof here should return the length of the string + null terminator */
	/* that's what we need to save in flash prob. */
	memcpy(&log_elem.u.str, msg, strlen(msg) + 1);
	return osMessageQueuePut(logf_queue, &log_elem, 0U, 0U);
}
...
osStatus_t logSensor(timestamp_t ts, board_id_t sensor_board_id,
	...
	case GPS: {
		...
		LOG_BUFFER_LEN, "%ld,%ld,%ld,%d,%ld,%d,%ld,%d,%hd,%hd\n",
				...
				gps_data->lon_decimal, gps_data->satellite, gps_data->altitude,
				gps_data->HDOP);
	}
		break;
	case BATTERY: {
...
osStatus_t logEstimatorVar(timestamp_t ts, state_est_data_t estimator_data) {
	...
	snprintf(log_entry.str, LOG_BUFFER_LEN, "%lu;%d;%ld,%ld,%ld\n", ts,
			ESTIMATOR_VAR, estimator_data.position_world[2],
			estimator_data.velocity_rocket[0],
			estimator_data.acceleration_rocket[0]);
...
osStatus_t logControllerOutput(timestamp_t ts, int32_t controller_output,
	...
	log_entry_t log_entry = { 0 };
	snprintf(log_entry.str, LOG_BUFFER_LEN, "%lu;%d;%ld,%ld,%ld\n", ts,
			CONTROLLER_OUTPUT, controller_output, reference_error,
			integrated_error);
...
osStatus_t logMotor(timestamp_t ts, int32_t desired_position,
	...
	snprintf(log_entry.str, LOG_BUFFER_LEN, "%lu;%d;%ld,%ld\n", ts,
			MOTOR_POSITION, desired_position, actual_position);

	return osMessageQueuePut(log_queue, &log_entry, 0U, 0U);
}

osStatus_t logBattery(timestamp_t ts, int32_t desired_position,
		int32_t actual_position) {
	log_entry_t log_entry = { 0 };
	snprintf(log_entry.str, LOG_BUFFER_LEN, "%lu;%d;%ld,%ld\n", ts,
			MOTOR_POSITION, desired_position, actual_position);
