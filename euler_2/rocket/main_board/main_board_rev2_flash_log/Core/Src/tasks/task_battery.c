...
void vTaskBattery(void *argument) {
  ...
  for (;;) {
    ...
    double current2 = ((double)adc_value[0] * (2.5 / 4096.0) - (3.3 * 0.107)) /
                      0.264;                                    // CURR2
    float supply_voltage = adc_value[1] * (2.5 / 4096) * 2;     // 3V3
    float battery_voltage = adc_value[2] * (2.5 / 4096) * 5.2;  // BAT
    double current1 = ((double)adc_value[3] * (2.5 / 4096.0) - (3.3 * 0.107)) /
                      0.264;  // CURR1
    ...
    if (counter < 50) {
      ...
      supp = 0;
      //			UsbPrint(
      //					"[BATTERY] Supply: %d Battery:
      //%d Current: %dmA, Consumption: %dmAh Power: %dmW\n",
      //					battery_data.supply,
      // battery_data.battery, battery_data.current, battery_data.consumption,
      //					battery_data.power);
