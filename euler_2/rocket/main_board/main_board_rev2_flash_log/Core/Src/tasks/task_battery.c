...
void vTaskBattery(void *argument) {
	...
	for (;;) {
		...
		if (counter < 50) {
			...
			supp = 0;
//			UsbPrint(
//					"[BATTERY] Supply: %d Battery: %d Current: %dmA, Consumption: %dmAh Power: %dmW\n",
//					battery_data.supply, battery_data.battery,
//					battery_data.current, battery_data.consumption,
//					battery_data.power);

			/* Log Battery Power */
			logSensorF(osKernelGetTickCount(), 1, BATTERY, &battery_data);
