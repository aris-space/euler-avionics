 ...
 * if necessary and waits for a Start command from Tracealyzer ("Start
 *Recording" button). This call is intentionally blocking! By calling
 *vTraceEnable with this option from the startup code, you start tracing at this
 ...
 * User Events can be used for very efficient logging from your application
 *code. It is very fast since the actual string formatting is done on the host
 *side, when the trace is displayed. The execution time is just some
 *microseconds on a 32-bit MCU.
 *
 ...
 *traceString as the argument. In snapshot you simply provide a char* as
 *argument.
 *
 * Snapshot: vTracePrintF(myChn, "my string: %s", str);
 * Streaming: vTracePrintF(myChn, "my string: %s", xTraceRegisterString(str));
 *
 * In snapshot mode you can specify 8-bit or 16-bit arguments to reduce RAM
 *usage: %hd -> 16 bit (h) signed integer (d). %bu -> 8 bit (b) unsigned integer
 *(u).
 *
 ...
 *
 * In snapshot mode you are limited to maximum 15 arguments, that must not
 *exceed 32 bytes in total (not counting the format string). If exceeded, the
 *recorder logs an internal error (displayed when opening the trace) and stops
 *recording.
 ******************************************************************************/
#if (TRC_CFG_SCHEDULING_ONLY == 0) && (TRC_CFG_INCLUDE_USER_EVENTS == 1)
void vTracePrintF(traceString chn, const char* fmt, ...);
#else
#define vTracePrintF(chn, fmt, ...)                                    \
  (void)(chn),                                                         \
      (void)(fmt) /* Comma operator is used to avoid "unused variable" \
                     compiler warnings in a single statement */
#endif
...
#else
#define vTraceVPrintF(chn, formatStr, vl)                                      \
  (void)(chn), (void)(formatStr),                                              \
      (void)(vl) /* Comma operator is used to avoid "unused variable" compiler \
...
#else
#define vTracePrint(chn, str)                                          \
  (void)(chn),                                                         \
      (void)(str) /* Comma operator is used to avoid "unused variable" \
                     compiler warnings in a single statement */
#endif
...
#else
#define xTraceRegisterString(x)                                                \
  ((void)(x), (traceString)0) /* Comma operator in parenthesis is used to      \
                                 avoid "unused variable" compiler warnings and \
                                 return 0 in a single statement */
#endif
 ...
 *
 * The trcConfig.h setting TRC_CFG_RECORDER_BUFFER_ALLOCATION allows for
 *selecting custom allocation (TRC_RECORDER_BUFFER_ALLOCATION_CUSTOM), which
 *allows you to control where the recorder trace buffer is allocated.
 *
 ...
 *
 * If using custom allocation of the trace buffer (i.e., your trcConfig.h has
 *the setting TRC_RECORDER_BUFFER_ALLOCATION_CUSTOM), this macro allows you to
 *declare the trace buffer in a portable way that works both in snapshot and
 *streaming.
 *
 ...
 * Example:
 *
 *   // GCC example: place myTraceBuffer in section .tz, defined in the .ld
 *file. TRC_ALLOC_CUSTOM_BUFFER(myTraceBuffer) __attribute__((section(".tz")));
 *
 *   int main(void)
 *   {
 *      ...
...
#define TRC_ALLOC_CUSTOM_BUFFER(bufname)                                      \
  char bufname[TRC_CFG_RTT_BUFFER_SIZE_UP]; /* Not static in this case, since \
                                               declared in user code */
#else
#define TRC_ALLOC_CUSTOM_BUFFER(bufname)                 \
  char bufname[(TRC_CFG_PAGED_EVENT_BUFFER_PAGE_COUNT) * \
 ...
 *		<Create Queue Q3>
 *
 *		// Only include FilterGroup0 and FilterGroup2, exclude
 *FilterGroup1 (Q1 and Q2) from the trace vTraceSetFilterMask( FilterGroup0 |
 *FilterGroup2 );
 *
 *		// Assign the default RTOS objects (e.g. Idle task) to
 *FilterGroup0 vTraceSetFilterGroup(FilterGroup0); <Start the RTOS scheduler>
 *
 ...
 * trcSnapshotConfig.h has a too small value (TRC_CFG_NTASK, TRC_CFG_NQUEUE,
 *etc).
 *
...
#define trcKERNEL_HOOKS_TASK_CREATE(SERVICE, CLASS, pxTCB)                    \
  ...
  prvTraceSetObjectName(TRACE_CLASS_TASK, TRACE_GET_TASK_NUMBER(pxTCB),       \
                        TRACE_GET_TASK_NAME(pxTCB));                          \
  prvTraceSetPriorityProperty(TRACE_CLASS_TASK, TRACE_GET_TASK_NUMBER(pxTCB), \
                              TRACE_GET_TASK_PRIORITY(pxTCB));                \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &               \
      CurrentFilterMask)                                                      \
    if (TRACE_GET_OBJECT_FILTER(TASK, pxTCB) & CurrentFilterMask)             \
      prvTraceStoreKernelCall(SERVICE, TRACE_CLASS_TASK,                      \
...
#define trcKERNEL_HOOKS_TASK_DELETE(SERVICE, SERVICE_NAME, SERVICE_PROP,      \
                                    pxTCB)                                    \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &               \
      CurrentFilterMask)                                                      \
    if (TRACE_GET_OBJECT_FILTER(TASK, pxTCB) & CurrentFilterMask)             \
  ...
  prvTraceStoreObjectNameOnCloseEvent(                                        \
      SERVICE_NAME, TRACE_GET_TASK_NUMBER(pxTCB), TRACE_CLASS_TASK);          \
  prvTraceStoreObjectPropertiesOnCloseEvent(                                  \
      SERVICE_PROP, TRACE_GET_TASK_NUMBER(pxTCB), TRACE_CLASS_TASK);          \
  prvTraceSetPriorityProperty(TRACE_CLASS_TASK, TRACE_GET_TASK_NUMBER(pxTCB), \
                              TRACE_GET_TASK_PRIORITY(pxTCB));                \
  prvTraceSetObjectState(TRACE_CLASS_TASK, TRACE_GET_TASK_NUMBER(pxTCB),      \
                         TASK_STATE_INSTANCE_NOT_ACTIVE);                     \
...
#define trcKERNEL_HOOKS_OBJECT_CREATE(SERVICE, CLASS, pxObject)              \
  ...
  prvMarkObjectAsUsed(TRACE_GET_OBJECT_TRACE_CLASS(CLASS, pxObject),         \
                      TRACE_GET_OBJECT_NUMBER(CLASS, pxObject));             \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &              \
      CurrentFilterMask)                                                     \
    if (TRACE_GET_OBJECT_FILTER(CLASS, pxObject) & CurrentFilterMask)        \
      prvTraceStoreKernelCall(SERVICE,                                       \
                              TRACE_GET_OBJECT_TRACE_CLASS(CLASS, pxObject), \
                              TRACE_GET_OBJECT_NUMBER(CLASS, pxObject));     \
  prvTraceSetObjectState(TRACE_GET_OBJECT_TRACE_CLASS(CLASS, pxObject),      \
                         TRACE_GET_OBJECT_NUMBER(CLASS, pxObject), 0);

/* This macro will remove the object and store it in the event buffer */
#undef trcKERNEL_HOOKS_OBJECT_DELETE
#define trcKERNEL_HOOKS_OBJECT_DELETE(SERVICE, SERVICE_NAME, SERVICE_PROP,   \
                                      CLASS, pxObject)                       \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &              \
      CurrentFilterMask)                                                     \
    if (TRACE_GET_OBJECT_FILTER(CLASS, pxObject) & CurrentFilterMask)        \
      prvTraceStoreKernelCall(SERVICE,                                       \
                              ...
                              TRACE_GET_OBJECT_NUMBER(CLASS, pxObject));     \
  prvTraceStoreObjectNameOnCloseEvent(                                       \
      SERVICE_NAME, TRACE_GET_OBJECT_NUMBER(CLASS, pxObject),                \
      TRACE_GET_OBJECT_TRACE_CLASS(CLASS, pxObject));                        \
  prvTraceStoreObjectPropertiesOnCloseEvent(                                 \
      SERVICE_PROP, TRACE_GET_OBJECT_NUMBER(CLASS, pxObject),                \
      TRACE_GET_OBJECT_TRACE_CLASS(CLASS, pxObject));                        \
  prvTraceFreeObjectHandle(TRACE_GET_OBJECT_TRACE_CLASS(CLASS, pxObject),    \
                           TRACE_GET_OBJECT_NUMBER(CLASS, pxObject));

/* This macro will create a call to a kernel service with a certain result, with
...
#define trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(SERVICE, CLASS, pxObject, \
                                                  param)                    \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &             \
      CurrentFilterMask)                                                    \
    if (TRACE_GET_OBJECT_FILTER(CLASS, pxObject) & CurrentFilterMask)       \
      prvTraceStoreKernelCallWithParam(                                     \
          SERVICE, TRACE_GET_OBJECT_TRACE_CLASS(CLASS, pxObject),           \
          TRACE_GET_OBJECT_NUMBER(CLASS, pxObject), (uint32_t)param);

/* This macro will create a call to a kernel service with a certain result, with
 * a null object and other value as parameter */
#undef trcKERNEL_HOOKS_KERNEL_SERVICE_NULL_OBJECT_WITH_PARAM
#define trcKERNEL_HOOKS_KERNEL_SERVICE_NULL_OBJECT_WITH_PARAM(             \
    SERVICE, TRACECLASS, param)                                            \
  if (TRACE_GET_TASK_FILTER(TRACE_GET_CURRENT_TASK()) & CurrentFilterMask) \
...
#define trcKERNEL_HOOKS_KERNEL_SERVICE_NULL_OBJECT_FROM_ISR(SERVICE,    \
                                                            TRACECLASS) \
  prvTraceStoreKernelCall(SERVICE, TRACECLASS, 0);
...
#define trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM_FROM_ISR(SERVICE, CLASS,  \
                                                           pxObject, param) \
  if (TRACE_GET_OBJECT_FILTER(CLASS, pxObject) & CurrentFilterMask)         \
    prvTraceStoreKernelCallWithParam(                                       \
        SERVICE, TRACE_GET_OBJECT_TRACE_CLASS(CLASS, pxObject),             \
...
#define trcKERNEL_HOOKS_KERNEL_SERVICE_NULL_OBJECT_WITH_PARAM_FROM_ISR( \
    SERVICE, TRACECLASS, param)                                         \
  prvTraceStoreKernelCallWithParam(SERVICE, TRACECLASS, 0, param);
...
#define trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_NUMERIC_PARAM_ONLY_FROM_ISR( \
    SERVICE, param)                                                      \
  prvTraceStoreKernelCallWithNumericParamOnly(SERVICE, (uint32_t)param);
...
#define trcKERNEL_HOOKS_SET_TASK_INSTANCE_FINISHED()            \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & \
    ...
    prvTraceSetTaskInstanceFinished(                            \
        TRACE_GET_TASK_NUMBER(TRACE_GET_CURRENT_TASK()));
...
/* This macro will update the internal tick counter and call
 * prvTracePortGetTimeStamp(0) to update the internal counters */
...
/* This macro will create an event indicating that the OS tick count has
 * increased */
...
/* This macro will create an event to indicate that a task has called a
 * wait/delay function */
...
/* This macro will create an event to indicate that a task has gotten its
 * priority changed */
#undef trcKERNEL_HOOKS_TASK_PRIORITY_CHANGE
#define trcKERNEL_HOOKS_TASK_PRIORITY_CHANGE(SERVICE, pxTCB, uxNewPriority) \
  if (TRACE_GET_OBJECT_FILTER(TASK, pxTCB) & CurrentFilterMask) {           \
    prvTraceStoreKernelCallWithParam(                                       \
        SERVICE, TRACE_CLASS_TASK, TRACE_GET_TASK_NUMBER(pxTCB),            \
        prvTraceGetPriorityProperty(TRACE_CLASS_TASK,                       \
                                    TRACE_GET_TASK_NUMBER(pxTCB)));         \
    prvTraceSetPriorityProperty(TRACE_CLASS_TASK,                           \
                                TRACE_GET_TASK_NUMBER(pxTCB),               \
                                (uint8_t)uxNewPriority);                    \
  }
...
void prvTraceStoreKernelCall(uint32_t eventcode, traceObjectClass objectClass,
                             uint32_t byteParam);
...
#define prvTraceStoreKernelCallWithNumericParamOnly(evtcode, param) \
  {}
#define prvTraceStoreKernelCallWithParam(evtcode, objectClass, objectNumber, \
                                         param)                              \
  {}
...
#define vTraceSetISRProperties(handle, name, priority)                      \
  (void)(handle), (void)(name),                                             \
      (void)(priority) /* Comma operator is used to avoid "unused variable" \
                          compiler warnings in a single statement */
...
#define xTraceSetISRProperties(name, priority)                               \
  ((void)(name), (void)(priority),                                           \
   (traceHandle)0) /* Comma operator in parenthesis is used to avoid "unused \
                      variable" compiler warnings and return 0 in a single   \
                      statement */
...
#else /*((TRC_CFG_INCLUDE_USER_EVENTS == 1) && (TRC_CFG_SCHEDULING_ONLY == \
         0))*/
...
#define xTraceRegisterChannelFormat(eventLabel, formatStr)                \
  ((void)(eventLabel), (void)(formatStr),                                 \
...
typedef struct {
  ...
  uint8_t channelBuffer[((TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE) + 3) &
                        0xFFFFFFFC]; /* 1 byte per slot, with padding for 4 byte
                                        alignment */
  uint8_t dataBuffer[(
      TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE)*4]; /* 4 bytes per slot */
...
#define TRACE_PROPERTY_NAME_GET(objectclass, objecthandle) \
  (const char*)(&RecorderDataPtr->ObjectPropertyTable      \
                     .objbytes[uiIndexOfObject(objecthandle, objectclass)])

#define TRACE_PROPERTY_OBJECT_STATE(objectclass, handle) \
  RecorderDataPtr->ObjectPropertyTable.objbytes          \
      [uiIndexOfObject(handle, objectclass) +            \
 ...
 *
 * Defining any actions needed in the stream port when the recording is
 *activated.
 *******************************************************************************/
...
#else
#define TRC_STREAM_PORT_ALLOCATE_EVENT(_type, _ptrData, _size) \
  _type _tmpArray[_size / sizeof(_type)];                      \
  _type* _ptrData = _tmpArray;
#define TRC_STREAM_PORT_ALLOCATE_EVENT_BLOCKING(_type, _ptrData, _size) \
  _type _tmpArray[_size / sizeof(_type)];                               \
  _type* _ptrData = _tmpArray;
#endif
...
#if (TRC_STREAM_PORT_USE_INTERNAL_BUFFER == 1)
#define TRC_STREAM_PORT_ALLOCATE_DYNAMIC_EVENT(_type, _ptrData, _size) \
  TRC_STREAM_PORT_ALLOCATE_EVENT(                                      \
      _type, _ptrData,                                                 \
      _size) /* We do the same thing as for non-dynamic event sizes */
#else
#define TRC_STREAM_PORT_ALLOCATE_DYNAMIC_EVENT(_type, _ptrData, _size) \
  _type _tmpArray[sizeof(largestEventType) / sizeof(_type)];           \
  _type* _ptrData = _tmpArray;
#endif
#endif
 ...
 *					with the number of bytes that was
 *received.
 *
 * Example:
 *
...
#error \
    "No definition for TRC_STREAM_PORT_READ_DATA (should be in trcStreamingPort.h)"
#endif
 ...
 * bytes were sent. A non-zero value should be returned if the streaming
 *interface returned an error (e.g. a closed socket), which results in the
 *recorder calling prvTraceWarning with the error code
 *PSF_WARNING_STREAM_PORT_WRITE.
 *
...
#error \
    "No definition for TRC_STREAM_PORT_WRITE_DATA (should be in trcStreamingPort.h)"
#endif
...
#define TRC_STREAM_PORT_ALLOCATE_FIELDS()                     \
  char _TzTraceData[(TRC_CFG_PAGED_EVENT_BUFFER_PAGE_COUNT) * \
                    (TRC_CFG_PAGED_EVENT_BUFFER_PAGE_SIZE)];
extern char _TzTraceData[(TRC_CFG_PAGED_EVENT_BUFFER_PAGE_COUNT) *
                         (TRC_CFG_PAGED_EVENT_BUFFER_PAGE_SIZE)];
#endif
...
#define TRC_STREAM_PORT_MALLOC()                                           \
  _TzTraceData = TRC_PORT_MALLOC((TRC_CFG_PAGED_EVENT_BUFFER_PAGE_COUNT) * \
                                 (TRC_CFG_PAGED_EVENT_BUFFER_PAGE_SIZE));
extern char* _TzTraceData;
#else
...
#define vTraceStoreKernelObjectName(a, b)                                   \
  (void)(a), (void)(b) /* Comma operator is used to avoid "unused variable" \
                          compiler warnings in a single statement */
#define xTraceRegisterChannelFormat(eventLabel, formatStr)                \
  ((void)(eventLabel), (void)(formatStr),                                 \
   0) /* Comma operator in parenthesis is used to avoid "unused variable" \
