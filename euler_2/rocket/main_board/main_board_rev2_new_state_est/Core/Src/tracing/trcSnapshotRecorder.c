...
#endif /*!defined TRC_CFG_INCLUDE_READY_EVENTS || TRC_CFG_INCLUDE_READY_EVENTS \
          == 1*/
...
/* Indicates if we are currently performing a context switch or just running
 * application code */
...
static void prvTraceGetChecksum(const char* pname, uint8_t* pcrc,
                                uint8_t* plength);
static void* prvTraceNextFreeEventBufferSlot(void);
static uint16_t prvTraceGetDTS(uint16_t param_maxDTS);
static traceString prvTraceOpenSymbol(const char* name,
                                      traceString userEventChannel);
static void prvTraceUpdateCounters(void);

void vTraceStoreMemMangEvent(uint32_t ecode, uint32_t address,
                             int32_t signed_size);
...
static void prvTraceUBHelper1(traceUBChannel channel, traceString eventLabel,
                              traceString formatLabel, va_list vl);
static void prvTraceUBHelper2(traceUBChannel channel, uint32_t* data,
                              uint32_t noOfSlots);
#endif /* (TRC_CFG_USE_SEPARATE_USER_EVENT_BUFFER == 1) */
#endif /* ((TRC_CFG_SCHEDULING_ONLY == 0) && (TRC_CFG_INCLUDE_USER_EVENTS == \
          1)) */
...
void vTraceSetRecorderDataBuffer(void* pRecorderData) {
  TRACE_ASSERT(pRecorderData != NULL,
               "vTraceSetRecorderDataBuffer, pRecorderData == NULL",
               TRC_UNUSED);
...
uint32_t uiTraceStart(void) {
  ...
  if (RecorderDataPtr == NULL) {
    TRACE_ASSERT(RecorderDataPtr != NULL,
                 "Recorder not initialized. Use vTraceEnable() instead!", 0);
    return 0;
  }

  if (RecorderDataPtr->recorderActive == 1) return 1; /* Already running */
 ...
 * We recommend using vTraceEnable instead.
 ******************************************************************************/
void vTraceStart(void) { (void)uiTraceStart(); }
 ...
 * Any error message is also presented when opening a trace file.
 ******************************************************************************/
const char* xTraceGetLastError(void) { return traceErrorMessage; }

/*******************************************************************************
 * vTraceClearError
 *
 * Removes any previous error message generated by recorder calling
 *prvTraceError. By calling this function, it may be possible to start/restart
 *the trace despite errors in the recorder, but there is no guarantee that the
 *trace recorder will work correctly in that case, depending on the type of
 *error.
 ******************************************************************************/
 ...
 * the data.
 ******************************************************************************/
void* xTraceGetTraceBuffer(void) { return RecorderDataPtr; }
 ...
 ******************************************************************************/
uint32_t uiTraceGetTraceBufferSize(void) { return sizeof(RecorderDataType); }
 ...
 *		}
 *****************************************************************************/
void vTraceInstanceFinishedNext(void) { prvTraceTaskInstanceFinish(0); }
 ...
 *iteration. The first instance ends at vTraceInstanceFinishedNow(), while the
 *second instance ends at the next xQueueReceive call.
 *
 *		while (1)
 *		{
 *          xQueueReceive(CommandQueue, &command, timeoutDuration);
 ...
 *      }
 *****************************************************************************/
void vTraceInstanceFinishedNow(void) { prvTraceTaskInstanceFinish(1); }
...
traceHandle xTraceSetISRProperties(const char* name, uint8_t priority) {
  ...
  TRACE_ASSERT(RecorderDataPtr != NULL,
               "Recorder not initialized, call vTraceEnable() first!",
               (traceHandle)0);
  TRACE_ASSERT(handle <= RecorderDataPtr->ObjectPropertyTable
                             .NumberOfObjectsPerClass[TRACE_CLASS_ISR],
               "xTraceSetISRProperties: Invalid value for handle", 0);
...
void vTraceStoreISRBegin(traceHandle handle) {
  ...
  if (RecorderDataPtr->recorderActive && handle_of_last_logged_task) {
    ...
    TRACE_ASSERT(handle != 0, "vTraceStoreISRBegin: Invalid ISR handle (NULL)",
                 TRC_UNUSED);
    TRACE_ASSERT(handle <= RecorderDataPtr->ObjectPropertyTable
                               .NumberOfObjectsPerClass[TRACE_CLASS_ISR],
                 "vTraceStoreISRBegin: Invalid ISR handle (> NISR)",
                 TRC_UNUSED);
...
void vTraceStoreISREnd(int pendingISR) {
  ...
  if (nISRactive == 0) {
    prvTraceError(
        "Unmatched call to vTraceStoreISREnd (nISRactive == 0, expected > 0)");
    return;
  }

  trcCRITICAL_SECTION_BEGIN();
  isPendingContextSwitch |=
      pendingISR; /* Is there a pending context switch right now? */
  nISRactive--;
  if (nISRactive > 0) {
    /* Return to another ISR */
    type = TS_ISR_RESUME;
    hnd8 = prvTraceGet8BitHandle(
        isrstack[nISRactive -
                 1]); /* isrstack[nISRactive] is the handle of the ISR we're
                         currently exiting. isrstack[nISRactive - 1] is the
                         handle of the ISR that was executing previously. */
 ...
 ******************************************************************************/
#if ((TRC_CFG_SCHEDULING_ONLY == 0) && (TRC_CFG_INCLUDE_USER_EVENTS == 1))
static uint8_t prvTraceUserEventFormat(const char* formatStr, va_list vl,
                                       uint8_t* buffer, uint8_t byteOffset) {
    ...
    if (formatStr[formatStrIndex] == '%') {
      ...
      if (formatStr[formatStrIndex] != '\0') {
        switch (formatStr[formatStrIndex]) {
          case 'd':
            i = writeInt32(buffer, i, (uint32_t)va_arg(vl, uint32_t));
            break;
          case 'x':
          case 'X':
          case 'u':
          ...
          case 'f':
            i = writeInt32(buffer, i, (uint32_t)va_arg(vl, double));
            break;
#endif
          case 'l':
            formatStrIndex++;
...
#if (TRC_CFG_INCLUDE_FLOAT_SUPPORT)
              case 'f':
                i = writeDouble(buffer, i, (double)va_arg(vl, double));
                break;
#else
              /* No support for floats, but attempt to store a float user event
              avoid a possible crash due to float reference. Instead store the
              data on uint_32 format (will not be displayed anyway). This is
              just to keep va_arg and i consistent. */
              case 'f':
                i = writeInt32(buffer, /* In this case, the value will not be
                                          shown anyway */
                               i, (uint32_t)va_arg(vl, double));

                i = writeInt32(
                    buffer, /* Do it twice, to write in total 8 bytes */
                    i, (uint32_t)va_arg(vl, double));
                break;
#endif
            }
          ...
          case 'h':
            ...
            switch (formatStr[formatStrIndex]) {
              case 'd':
                i = writeInt16(buffer, i, (uint16_t)va_arg(vl, uint32_t));
                break;
              case 'u':
                i = writeInt16(buffer, i, (uint16_t)va_arg(vl, uint32_t));
                break;
          ...
          case 'b':
            ...
            switch (formatStr[formatStrIndex]) {
              case 'd':
                i = writeInt8(buffer, i, (uint8_t)va_arg(vl, uint32_t));
                break;
              case 'u':
                i = writeInt8(buffer, i, (uint8_t)va_arg(vl, uint32_t));
                break;
 ...
 *
 * Clears a number of items in the channel buffer, starting from
 *nextSlotToWrite.
 ******************************************************************************/
#if ((TRC_CFG_SCHEDULING_ONLY == 0) && (TRC_CFG_INCLUDE_USER_EVENTS == 1) && \
     (TRC_CFG_USE_SEPARATE_USER_EVENT_BUFFER == 1))
static void prvTraceClearChannelBuffer(uint32_t count) {
  uint32_t slots;

  TRACE_ASSERT(
      (TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE) >= count,
      "prvTraceClearChannelBuffer: TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE is "
      "too small to handle this event.",
      TRC_UNUSED);

  /* Check if we're close to the end of the buffer */
  if (RecorderDataPtr->userEventBuffer.nextSlotToWrite + count >
      (TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE)) {
    slots = (TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE)-RecorderDataPtr
                ->userEventBuffer
                .nextSlotToWrite; /* Number of slots before end of buffer */
    (void)memset(
        &RecorderDataPtr->userEventBuffer
             .channelBuffer[RecorderDataPtr->userEventBuffer.nextSlotToWrite],
        0, slots);
    (void)memset(&RecorderDataPtr->userEventBuffer.channelBuffer[0], 0,
                 (count - slots));
  } else
    (void)memset(
        &RecorderDataPtr->userEventBuffer
...
static void prvTraceCopyToDataBuffer(uint32_t* data, uint32_t count) {
  ...
  TRACE_ASSERT(
      ...
      "prvTraceCopyToDataBuffer: TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE is "
      "too small to handle this event.",
      TRC_UNUSED);
  /* Check if we're close to the end of the buffer */
  if (RecorderDataPtr->userEventBuffer.nextSlotToWrite + count >
    ...
    (void)memcpy(
        &RecorderDataPtr->userEventBuffer
             .dataBuffer[RecorderDataPtr->userEventBuffer.nextSlotToWrite * 4],
        data, slots * 4);
    (void)memcpy(&RecorderDataPtr->userEventBuffer.dataBuffer[0], data + slots,
                 (count - slots) * 4);
  } else {
    (void)memcpy(
        &RecorderDataPtr->userEventBuffer
...
static void prvTraceUBHelper1(traceUBChannel channel, traceString eventLabel,
  ...
  if (channel == 0) {
    ...
    byteOffset =
        (uint8_t)(byteOffset + 4); /* Also need room for channel and format */
    ((uint16_t*)data)[2] = eventLabel;
    ((uint16_t*)data)[3] = formatLabel;
  }

  noOfSlots = prvTraceUserEventFormat(
      (char*)&(RecorderDataPtr->SymbolTable.symbytes[formatLabel + 4]), vl,
      (uint8_t*)data, byteOffset);
...
static void prvTraceUBHelper2(traceUBChannel channel, uint32_t* data,
  ...
  TRACE_ASSERT((TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE) >= noOfSlots,
               "prvTraceUBHelper2: TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE is "
               "too small to handle this event.",
               TRC_UNUSED);
  ...
  prvTraceCopyToDataBuffer(
      data, noOfSlots); /* Will wrap around the data if necessary */

  old_nextSlotToWrite =
      RecorderDataPtr->userEventBuffer
          .nextSlotToWrite; /* Save the index that we want to write the channel
                               data at when we're done */
  RecorderDataPtr->userEventBuffer.nextSlotToWrite =
      (RecorderDataPtr->userEventBuffer.nextSlotToWrite + noOfSlots) %
      (TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE); /* Make sure we never end up
                                                    outside the buffer */

  /* Write to the channel buffer to indicate that this user event is ready to be
   * used */
  if (channel != 0) {
    RecorderDataPtr->userEventBuffer.channelBuffer[old_nextSlotToWrite] =
        channel;
  } else {
...
#if ((TRC_CFG_SCHEDULING_ONLY == 0) && (TRC_CFG_INCLUDE_USER_EVENTS == 1) && \
     (TRC_CFG_USE_SEPARATE_USER_EVENT_BUFFER == 1))
traceUBChannel xTraceRegisterUBChannel(traceString channel,
  ...
  TRACE_ASSERT(formatStr != 0, "xTraceRegisterChannelFormat: formatStr == 0",
               (traceUBChannel)0);

  trcCRITICAL_SECTION_BEGIN();
  for (i = 1; i <= (TRC_CFG_UB_CHANNELS);
       i++) /* Size of the channels buffer is TRC_CFG_UB_CHANNELS + 1. Index 0
               is unused. */
  {
    if (RecorderDataPtr->userEventBuffer.channels[i].name == 0 &&
        RecorderDataPtr->userEventBuffer.channels[i].defaultFormat == 0) {
      /* Found empty slot */
    ...
    if (RecorderDataPtr->userEventBuffer.channels[i].name == channel &&
        RecorderDataPtr->userEventBuffer.channels[i].defaultFormat ==
            formatStr) {
      /* Found a match */
      retVal = (traceUBChannel)i;
      break;
...
void vTraceUBData(traceUBChannel channelPair, ...) {
  ...
  TRACE_ASSERT(channelPair != 0, "vTraceUBData: Not a valid traceUBChannel!",
               TRC_UNUSED);
...
/* Extracts the channel name and format string from the traceUBChannel, then
 * calls prvTraceUBHelper1. */
#if ((TRC_CFG_SCHEDULING_ONLY == 0) && (TRC_CFG_INCLUDE_USER_EVENTS == 1) && \
     (TRC_CFG_USE_SEPARATE_USER_EVENT_BUFFER == 1))
void vTraceUBData_Helper(traceUBChannel channelPair, va_list vl) {
  ...
  TRACE_ASSERT(channelPair != 0, "vTraceUBData_Helper: channelPair == 0",
               TRC_UNUSED);
  TRACE_ASSERT(channelPair <= (TRC_CFG_UB_CHANNELS),
               "vTraceUBData_Helper: ", TRC_UNUSED);

  channel = RecorderDataPtr->userEventBuffer.channels[channelPair].name;
  formatStr =
      RecorderDataPtr->userEventBuffer.channels[channelPair].defaultFormat;
...
void vTraceUBEvent(traceUBChannel channelPair) {
  ...
  TRACE_ASSERT(channelPair <= (TRC_CFG_UB_CHANNELS),
               "vTraceUBEvent: ", TRC_UNUSED);

  prvTraceUBHelper2(channelPair, data,
                    1); /* Only need one slot for timestamp */
}
#endif
 ...
 *				 ch, (double)adc_reading/(double)scale);
 *
 * Calling xTraceRegisterString multiple times will not create duplicate
 *entries, but it is of course faster to just do it once, and then keep the
 *handle for later use. If you don't have any data arguments, only a text
 *label/string, it is better to use vTracePrint - it is faster.
 *
...
void vTraceVPrintF(traceString eventLabel, const char* formatStr, va_list vl) {
  ...
  TRACE_ASSERT(formatStr != NULL, "vTraceVPrintF: formatStr == NULL",
               TRC_UNUSED);
  ...
  if (RecorderDataPtr->recorderActive && handle_of_last_logged_task) {
    ...
    noOfSlots =
        prvTraceUserEventFormat(formatStr, vl, (uint8_t*)tempDataBuffer, 4);
    ...
    if (RecorderDataPtr->recorderActive) {
      ...
      if (RecorderDataPtr->nextFreeIndex + noOfSlots >
          RecorderDataPtr->maxEvents) {
#if (TRC_CFG_SNAPSHOT_MODE == TRC_SNAPSHOT_MODE_RING_BUFFER)
        (void)memset(
            &RecorderDataPtr->eventData[RecorderDataPtr->nextFreeIndex * 4], 0,
...
#if (TRC_CFG_SNAPSHOT_MODE == TRC_SNAPSHOT_MODE_RING_BUFFER)
          /* We have reached the end, but this is a ring buffer. Start from the
           * beginning again. */
          RecorderDataPtr->bufferIsFull = 1;
          RecorderDataPtr->nextFreeIndex = 0;
#else
...
traceString xTraceRegisterString(const char* label) {
  TRACE_ASSERT(label != NULL, "xTraceRegisterString: label == NULL",
               (traceString)0);
  TRACE_ASSERT(RecorderDataPtr != NULL,
               "Recorder not initialized, call vTraceEnable() first!",
               (traceHandle)0);
...
#if ((!defined TRC_CFG_INCLUDE_READY_EVENTS) || \
     (TRC_CFG_INCLUDE_READY_EVENTS == 1))

void prvTraceSetReadyEventsEnabled(int status) { readyEventsEnabled = status; }
...
void prvTraceStoreTaskReady(traceHandle handle) {
  ...
  TRACE_ASSERT(handle <= (TRC_CFG_NTASK),
               "prvTraceStoreTaskReady: Invalid value for handle", TRC_UNUSED);
...
void prvTraceStoreLowPower(uint32_t flag) {
  ...
  TRACE_ASSERT(flag <= 1, "prvTraceStoreLowPower: Invalid flag value",
               TRC_UNUSED);
  ...
  if (RecorderDataPtr->recorderActive) {
    ...
    if (lp != NULL) {
      lp->type = (uint8_t)(LOW_POWER_BEGIN +
                           (uint8_t)flag); /* BEGIN or END depending on flag */
      lp->dts = dts;
      prvTraceUpdateCounters();
    }
  }
 ...
 * for size and address respectively. The event code parameter (ecode) is
 *applied to the first record (size) and the following address record gets event
...
void vTraceStoreMemMangEvent(uint32_t ecode, uint32_t address,
  ...
  if (address != 0) heapMemUsage += (uint32_t)signed_size;
 ...
 * is called by the hooks in trcKernelHooks.h (see trcKernelPort.h for event
 *codes).
 ******************************************************************************/
#if (TRC_CFG_SCHEDULING_ONLY == 0)
void prvTraceStoreKernelCall(uint32_t ecode, traceObjectClass objectClass,
  ...
  TRACE_ASSERT(ecode < 0xFF, "prvTraceStoreKernelCall: ecode >= 0xFF",
               TRC_UNUSED);
  TRACE_ASSERT(objectClass < TRACE_NCLASSES,
               "prvTraceStoreKernelCall: objectClass >= TRACE_NCLASSES",
               TRC_UNUSED);
  TRACE_ASSERT(objectNumber <= RecorderDataPtr->ObjectPropertyTable
                                   .NumberOfObjectsPerClass[objectClass],
               "prvTraceStoreKernelCall: Invalid value for objectNumber",
               TRC_UNUSED);
...
void prvTraceStoreKernelCallWithParam(uint32_t evtcode,
  ...
  TRACE_ASSERT(evtcode < 0xFF,
               "prvTraceStoreKernelCallWithParam: evtcode >= 0xFF", TRC_UNUSED);
  TRACE_ASSERT(
      objectClass < TRACE_NCLASSES,
      "prvTraceStoreKernelCallWithParam: objectClass >= TRACE_NCLASSES",
      TRC_UNUSED);
  TRACE_ASSERT(
      objectNumber <= RecorderDataPtr->ObjectPropertyTable
                          .NumberOfObjectsPerClass[objectClass],
      "prvTraceStoreKernelCallWithParam: Invalid value for objectNumber",
      TRC_UNUSED);
...
void prvTraceStoreKernelCallWithNumericParamOnly(uint32_t evtcode,
  ...
  TRACE_ASSERT(
      evtcode < 0xFF,
      "prvTraceStoreKernelCallWithNumericParamOnly: Invalid value for evtcode",
      TRC_UNUSED);
...
void prvTraceStoreTaskswitch(traceHandle task_handle) {
  ...
  TRACE_ASSERT(task_handle <= (TRC_CFG_NTASK),
               "prvTraceStoreTaskswitch: Invalid value for task_handle",
               TRC_UNUSED);

  trcCRITICAL_SECTION_BEGIN_ON_CORTEX_M_ONLY();

  if ((task_handle != handle_of_last_logged_task) &&
      (RecorderDataPtr->recorderActive)) {
#if (TRC_CFG_INCLUDE_ISR_TRACING == 1)
    ...
    if (ts != NULL) {
      ...
      prvTraceSetObjectState(TRACE_CLASS_TASK, handle_of_last_logged_task,
                             TASK_STATE_INSTANCE_ACTIVE);
...
void prvTraceStoreObjectNameOnCloseEvent(uint8_t evtcode, traceHandle handle,
  ...
  TRACE_ASSERT(
      objectclass < TRACE_NCLASSES,
      "prvTraceStoreObjectNameOnCloseEvent: objectclass >= TRACE_NCLASSES",
      TRC_UNUSED);
  TRACE_ASSERT(handle <= RecorderDataPtr->ObjectPropertyTable
                             .NumberOfObjectsPerClass[objectclass],
               "prvTraceStoreObjectNameOnCloseEvent: Invalid value for handle",
               TRC_UNUSED);
...
void prvTraceStoreObjectPropertiesOnCloseEvent(uint8_t evtcode,
  ...
  TRACE_ASSERT(objectclass < TRACE_NCLASSES,
               "prvTraceStoreObjectPropertiesOnCloseEvent: objectclass >= "
               "TRACE_NCLASSES",
               TRC_UNUSED);
  TRACE_ASSERT(
      handle <= RecorderDataPtr->ObjectPropertyTable
                    .NumberOfObjectsPerClass[objectclass],
      "prvTraceStoreObjectPropertiesOnCloseEvent: Invalid value for handle",
      TRC_UNUSED);
...
void prvTraceSetPriorityProperty(uint8_t objectclass, traceHandle id,
                                 uint8_t value) {
  TRACE_ASSERT(objectclass < TRACE_NCLASSES,
               "prvTraceSetPriorityProperty: objectclass >= TRACE_NCLASSES",
               TRC_UNUSED);
  TRACE_ASSERT(id <= RecorderDataPtr->ObjectPropertyTable
                         .NumberOfObjectsPerClass[objectclass],
...
void prvTraceSetObjectState(uint8_t objectclass, traceHandle id,
                            uint8_t value) {
  TRACE_ASSERT(objectclass < TRACE_NCLASSES,
               "prvTraceSetObjectState: objectclass >= TRACE_NCLASSES",
               TRC_UNUSED);
  TRACE_ASSERT(id <= RecorderDataPtr->ObjectPropertyTable
                         .NumberOfObjectsPerClass[objectclass],
...
void prvTraceSetTaskInstanceFinished(traceHandle handle) {
  TRACE_ASSERT(handle <= RecorderDataPtr->ObjectPropertyTable
                             .NumberOfObjectsPerClass[TRACE_CLASS_TASK],
               "prvTraceSetTaskInstanceFinished: Invalid value for handle",
               TRC_UNUSED);
 ...
 * RecorderData
 *
 * The main data structure in snapshot mode, when using the default static
 *memory allocation (TRC_RECORDER_BUFFER_ALLOCATION_STATIC). The recorder uses a
 *pointer RecorderDataPtr to access the data, to also allow for dynamic or
 *custom data allocation (see TRC_CFG_RECORDER_BUFFER_ALLOCATION).
 ******************************************************************************/
#if (TRC_CFG_RECORDER_BUFFER_ALLOCATION == \
...
void prvTraceInitTraceData() {
  if (RecorderDataPtr == NULL) {
...
#elif (TRC_CFG_RECORDER_BUFFER_ALLOCATION == \
    ...
    if (!RecorderDataPtr) {
      prvTraceError(
          "Recorder data pointer not set! Use vTraceSetRecorderDataBuffer().");
      return;
    }
#endif
  } else {
  ...
  RecorderDataPtr->isrTailchainingThreshold =
  ...
  RecorderDataPtr->userEventBuffer.numberOfSlots =
      (TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE);
  ...
  might be a risk of accidental duplicates of the start/end markers, e.g., due
  to compiler optimizations.

  The below initialization of the start marker is therefore made in reverse
  order and the fields are volatile to ensure this assignment order. This to
  avoid any chance of accidental duplicates of this elsewhere in memory.
  ...
  if (traceErrorMessage != NULL) {
    // An error was detected before vTraceEnable was called, make sure this is
    // stored in the trace data.
...
void* prvTraceNextFreeEventBufferSlot(void) {
  ...
  return (
      void*)(&RecorderDataPtr->eventData[RecorderDataPtr->nextFreeIndex * 4]);
}

uint16_t uiIndexOfObject(traceHandle objecthandle, uint8_t objectclass) {
  TRACE_ASSERT(objectclass < TRACE_NCLASSES,
               "uiIndexOfObject: Invalid value for objectclass", 0);
  TRACE_ASSERT(objecthandle > 0 &&
                   objecthandle <= RecorderDataPtr->ObjectPropertyTable
                                       .NumberOfObjectsPerClass[objectclass],
  ...
  if ((objectclass < TRACE_NCLASSES) && (objecthandle > 0) &&
      (objecthandle <= RecorderDataPtr->ObjectPropertyTable
                           .NumberOfObjectsPerClass[objectclass])) {
    return (uint16_t)(
        RecorderDataPtr->ObjectPropertyTable.StartIndexOfClass[objectclass] +
        (RecorderDataPtr->ObjectPropertyTable
             .TotalPropertyBytesPerClass[objectclass] *
         (objecthandle - 1)));
  }

  prvTraceError(
      "Object table lookup with invalid object handle or object class!");
  return 0;
}
...
traceHandle prvTraceGetObjectHandle(traceObjectClass objectclass) {
  ...
  TRACE_ASSERT(objectclass < TRACE_NCLASSES,
               "prvTraceGetObjectHandle: Invalid value for objectclass",
               (traceHandle)0);
  ...
  if (objectHandleStacks.objectHandles[indexOfHandle] == 0) {
    ...
    objectHandleStacks.objectHandles[indexOfHandle] = (traceHandle)(
        1 + indexOfHandle - objectHandleStacks.lowestIndexOfClass[objectclass]);
  }
...
void prvTraceFreeObjectHandle(traceObjectClass objectclass,
  ...
  TRACE_ASSERT(objectclass < TRACE_NCLASSES,
               "prvTraceFreeObjectHandle: Invalid value for objectclass",
               TRC_UNUSED);
  TRACE_ASSERT(
      handle > 0 && handle <= RecorderDataPtr->ObjectPropertyTable
                                  .NumberOfObjectsPerClass[objectclass],
 ...
 ******************************************************************************/
void prvTraceSetObjectName(traceObjectClass objectclass, traceHandle handle,
                           const char* name) {
  static uint16_t idx;
  ...
  if (handle > RecorderDataPtr->ObjectPropertyTable
                   .NumberOfObjectsPerClass[objectclass]) {
    /* ERROR */
    prvTraceError(pszTraceGetErrorNotEnoughHandles(objectclass));
  } else {
    ...
    if (traceErrorMessage == NULL) {
      prvStrncpy(
          (char*)&(RecorderDataPtr->ObjectPropertyTable.objbytes[idx]), name,
...
traceString prvTraceOpenSymbol(const char* name, traceString userEventChannel) {
  ...
  TRACE_ASSERT(name != NULL, "prvTraceOpenSymbol: name == NULL",
               (traceString)0);
...
void prvCheckDataToBeOverwrittenForMultiEntryEvents(uint8_t nofEntriesToCheck) {
  ...
  while (i < nofEntriesToCheck) {
    ...
    if ((RecorderDataPtr->eventData[e * 4] > USER_EVENT) &&
      ...
      uint8_t nDataEvents =
          (uint8_t)(RecorderDataPtr->eventData[e * 4] - USER_EVENT);
      if ((e + nDataEvents) < RecorderDataPtr->maxEvents) {
        (void)memset(&RecorderDataPtr->eventData[e * 4], 0,
                     (size_t)(4 + 4 * nDataEvents));
      }
...
uint16_t prvTraceGetDTS(uint16_t param_maxDTS) {
  ...
  TRACE_ASSERT(param_maxDTS == 0xFF || param_maxDTS == 0xFFFF,
               "prvTraceGetDTS: Invalid value for param_maxDTS", 0);

  if (RecorderDataPtr->frequency == 0) {
    if (timestampFrequency != 0) {
      /* If to override default TRC_HWTC_FREQ_HZ value with value set by
       * vTraceSetFrequency */
  ...
  if (RecorderDataPtr->frequency > 0) {
    ...
    if (RecorderDataPtr->absTimeLastEvent >= RecorderDataPtr->frequency) {
      /* RecorderDataPtr->absTimeLastEvent is more than or equal to 1 second,
       * but always less than 2 seconds */
      RecorderDataPtr->absTimeLastEventSecond++;
      RecorderDataPtr->absTimeLastEvent -= RecorderDataPtr->frequency;
      /* RecorderDataPtr->absTimeLastEvent is now less than 1 second */
    }
  } else {
    /* Special case if the recorder has not yet started (frequency may be
     * uninitialized, i.e., zero) */
    RecorderDataPtr->absTimeLastEvent = timestamp;
  }

  /* If the dts (time since last event) does not fit in event->dts (only 8 or 16
   * bits) */
  if (dts > param_maxDTS) {
 ...
 * zero-termination
 ******************************************************************************/
traceString prvTraceLookupSymbolTableEntry(const char* name, uint8_t crc6,
                                           uint8_t len, traceString chn) {
  uint16_t i = RecorderDataPtr->SymbolTable.latestEntryOfChecksum[crc6];

  TRACE_ASSERT(name != NULL, "prvTraceLookupSymbolTableEntry: name == NULL",
               (traceString)0);
  TRACE_ASSERT(len != 0, "prvTraceLookupSymbolTableEntry: len == 0",
               (traceString)0);
  ...
  while (i != 0) {
    if (RecorderDataPtr->SymbolTable.symbytes[i + 2] == (chn & 0x00FF)) {
      if (RecorderDataPtr->SymbolTable.symbytes[i + 3] == (chn / 0x100)) {
        if (RecorderDataPtr->SymbolTable.symbytes[i + 4 + len] == '\0') {
          if (strncmp((char*)(&RecorderDataPtr->SymbolTable.symbytes[i + 4]),
                      name, len) == 0) {
            break; /* found */
          }
        }
    ...
    i = (uint16_t)(RecorderDataPtr->SymbolTable.symbytes[i] +
                   (RecorderDataPtr->SymbolTable.symbytes[i + 1] * 0x100));
  }
  return i;
}
 ...
 * zero-termination
 ******************************************************************************/
uint16_t prvTraceCreateSymbolTableEntry(const char* name, uint8_t crc6,
                                        uint8_t len, traceString channel) {
  uint16_t ret = 0;
  ...
  if (RecorderDataPtr->SymbolTable.nextFreeSymbolIndex + len + 4 >=
      (TRC_CFG_SYMBOL_TABLE_SIZE)) {
    prvTraceError(
        "Symbol table full. Increase TRC_CFG_SYMBOL_TABLE_SIZE in trcConfig.h");
    ret = 0;
  } else {
    RecorderDataPtr->SymbolTable
        .symbytes[RecorderDataPtr->SymbolTable.nextFreeSymbolIndex] = (uint8_t)(
        RecorderDataPtr->SymbolTable.latestEntryOfChecksum[crc6] & 0x00FF);

    RecorderDataPtr->SymbolTable
        .symbytes[RecorderDataPtr->SymbolTable.nextFreeSymbolIndex + 1] =
        (uint8_t)(RecorderDataPtr->SymbolTable.latestEntryOfChecksum[crc6] /
                  0x100);

    RecorderDataPtr->SymbolTable
        .symbytes[RecorderDataPtr->SymbolTable.nextFreeSymbolIndex + 2] =
        (uint8_t)(channel & 0x00FF);

    RecorderDataPtr->SymbolTable
        .symbytes[RecorderDataPtr->SymbolTable.nextFreeSymbolIndex + 3] =
        (uint8_t)(channel / 0x100);

    /* set name (bytes 4...4+len-1) */
    prvStrncpy(
        (char*)&(RecorderDataPtr->SymbolTable.symbytes
                     [RecorderDataPtr->SymbolTable.nextFreeSymbolIndex + 4]),
        name, len);

    /* Set zero termination (at offset 4+len) */
    RecorderDataPtr->SymbolTable
        .symbytes[RecorderDataPtr->SymbolTable.nextFreeSymbolIndex + 4 + len] =
        '\0';
...
void prvTracePortGetTimeStamp(uint32_t* pTimestamp) {
...
#else /*TRC_HWTC_TYPE == TRC_OS_TIMER_INCR || TRC_HWTC_TYPE == \
         TRC_OS_TIMER_DECR*/
  /* Free running timer */
  static uint32_t last_hwtc_rest = 0;
  uint32_t diff = 0;
  uint32_t diff_scaled = 0;
#endif /*TRC_HWTC_TYPE == TRC_OS_TIMER_INCR || TRC_HWTC_TYPE == \
          TRC_OS_TIMER_DECR*/
  ...
  /* Retrieve TRC_HWTC_COUNT only once since the same value should be used all
   * throughout this function. */
#if (TRC_HWTC_TYPE == TRC_OS_TIMER_INCR || \
     TRC_HWTC_TYPE == TRC_FREE_RUNNING_32BIT_INCR)
...
#error "TRC_HWTC_TYPE has unexpected value"
  ...
  /* The Win32 port uses ulGetRunTimeCounterValue for timestamping, which in
  turn uses QueryPerformanceCounter. That function is not always reliable when
  used over multiple threads. We must therefore handle rare cases where the
  timestamp is less than the previous. In practice, this should "never" roll
  over since the performance counter is 64 bit wide. */
  ...
  if (traceTickCount == last_traceTickCount && hwtc_count < last_hwtc_count) {
    /* A trace tick has occurred but not been executed by the kernel, so we
     * compensate manually. */
    traceTickCount++;
  }
  ...
  if (pTimestamp) {
    /* Get timestamp from trace ticks. Scale down the period to avoid unwanted
     * overflows. */
    last_timestamp = traceTickCount * ((TRC_HWTC_PERIOD) / (TRC_HWTC_DIVISOR));
    /* Increase timestamp by (hwtc_count + "lost hardware ticks from scaling
     * down period") / TRC_HWTC_DIVISOR. */
    last_timestamp += (hwtc_count + traceTickCount * ((TRC_HWTC_PERIOD) %
                                                      (TRC_HWTC_DIVISOR))) /
                      (TRC_HWTC_DIVISOR);
  }
  ...
  /* This part handles free running clocks that can be scaled down to avoid too
  large DTS values. Without this, the scaled timestamp will incorrectly wrap at
  (2^32 / TRC_HWTC_DIVISOR) ticks. The scaled timestamp returned from this
  function is supposed to go from 0 -> 2^32, which in real time would represent
  (0 -> 2^32 * TRC_HWTC_DIVISOR) ticks. */

  /* First we see how long time has passed since the last timestamp call, and we
   * also add the ticks that was lost when we scaled down the last time. */
  diff = (hwtc_count - last_hwtc_count) + last_hwtc_rest;

  /* Scale down the diff */
  diff_scaled = diff / (TRC_HWTC_DIVISOR);

  /* Find out how many ticks were lost when scaling down, so we can add them the
   * next time */
