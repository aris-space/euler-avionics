...
void process_measurements(timestamp_t t, kf_state_t *kf_state,
	...
	for (int i = 0; i < NUM_SENSORBOARDS; i++) {
		...
		/* imu */
		if (state_est_meas->imu_data[i].ts
				> state_est_meas_prior->imu_data[i].ts) {
	...
	sensor_elimination_by_stdev(NUMBER_MEASUREMENTS, temp_meas,
			temp_meas_active);
...
void sensor_elimination_by_extrapolation(timestamp_t t, int32_t n,
		float measurements[n], bool measurement_active[n],
	...
	if (extrapolation_rolling_memory->memory_length >= MAX_LENGTH_ROLLING_MEMORY) {
		...
		for (int i = 0; i <= EXTRAPOLATION_POLYFIT_DEGREE; ++i) {
			x_priori += extrapolation_rolling_memory->polyfit_coeffs[i]
					* pow(t, i);
		}
	...
	if (num_active > 0) {
		...
		for (int i = 0; i < n; ++i) {
			if (measurement_active[i]) {
				extrapolation_rolling_memory->timestamps[idx_active] =
						(float) t;
