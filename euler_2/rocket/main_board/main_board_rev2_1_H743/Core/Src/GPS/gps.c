...
uint8_t _parse_data(struct gps_device *dev, uint8_t *data) {
  if (data[22] == 'N' || data[22] == 'S') {
    ...
    dev->data.lat_decimal =
        (data[13] * 1e7 + data[14] * 1e6 + data[16] * 1e5 + data[17] * 1e4 +
         data[18] * 1e3 + data[19] * 1e2 + data[20]) /
        6;

    dev->data.lon_deg = data[25] * 10 + data[26];
    dev->data.lon_decimal =
        (data[27] * 1e7 + data[28] * 1e6 + data[30] * 1e5 + data[31] * 1e4 +
         data[32] * 1e3 + data[33] * 1e2 + data[34]) /
        6;
