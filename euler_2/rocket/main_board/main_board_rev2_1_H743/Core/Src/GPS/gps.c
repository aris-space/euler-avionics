...
uint32_t _gps_search_start(uint8_t *arr) {
	for (int i = 3; i < BUFFER_SIZE - DATA_SIZE; i++) {
		if (arr[i - 2] == ubxgga[0] && arr[i - 1] == ubxgga[1]
				&& arr[i] == ubxgga[2])
			return i + 1;
	}
	return 0;
...
uint8_t _parse_data(struct gps_device *dev, uint8_t *data) {
	...
	if (data[22] == 'N' || data[22] == 'S') {
		...
		dev->data.lat_decimal = (data[13] * 1e7 + data[14] * 1e6
				+ data[16] * 1e5 + data[17] * 1e4 + data[18] * 1e3
				+ data[19] * 1e2 + data[20]) / 6;

		dev->data.lon_deg = data[25] * 10 + data[26];
		dev->data.lon_decimal = (data[27] * 1e7 + data[28] * 1e6
				+ data[30] * 1e5 + data[31] * 1e4 + data[32] * 1e3
				+ data[33] * 1e2 + data[34]) / 6;
...
uint8_t gps_read_sensor(struct gps_device *dev) {
	...
	if (data_start > 0)
		memcpy(gps_gga[dev->id], gps_data[dev->id] + data_start, DATA_SIZE);
