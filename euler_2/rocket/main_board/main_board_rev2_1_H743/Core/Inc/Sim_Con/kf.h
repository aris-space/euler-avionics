...
#define NUMBER_PROCESS_NOISE \
  1 /* NUMBER_STATES x NUMBER_PROCESS_NOISE -> G Matrix */
#define NUMBER_MEASUREMENTS \
  NUM_BARO            /* NUMBER_MEASUREMENTS x NUMBER_STATES -> H Matrix */
...
typedef struct {
  ...
  float P_est[NUMBER_STATES]
             [NUMBER_STATES];     // estimated covariance (posteriori)
  float x_priori[NUMBER_STATES];  // priori state (priori)
  float P_priori[NUMBER_STATES][NUMBER_STATES];  // priori covariance (priori)
  float P_priori_inv[NUMBER_STATES]
                    [NUMBER_STATES];  // inverted priori covariance
