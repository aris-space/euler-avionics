...
void matmul(int32_t m, int32_t n, int32_t o, float A[m][n], float B[n][o],
		float C[m][o], bool reset) {
	if (reset) {
...
/* https://www.geeksforgeeks.org/adjoint-inverse-matrix/ */
bool inverse(int32_t dim, float A[dim][dim], float A_inv[dim][dim],
		float lambda) {
...
bool pseudo_inverse(int32_t m, int32_t n, float A[m][n], float A_inv[n][m],
		float lambda) {
	float A_T[n][m];
	transpose(m, n, A, A_T);
	...
	for (ii = 0; ii < (order + 1); ii++) {
		...
		if (x != 0) {
			for (kk = 0; kk < (2 * (order + 1)); kk++) {
				A[(ii * (2 * (order + 1))) + kk] = A[(ii * (2 * (order + 1)))
						+ kk] / x;
			}
