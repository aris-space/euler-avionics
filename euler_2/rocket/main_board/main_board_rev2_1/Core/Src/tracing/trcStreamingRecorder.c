...
uint32_t TotalBytesRemaining_LowWaterMark =
    (TRC_CFG_PAGED_EVENT_BUFFER_PAGE_COUNT) *
    (TRC_CFG_PAGED_EVENT_BUFFER_PAGE_SIZE);
uint32_t TotalBytesRemaining = (TRC_CFG_PAGED_EVENT_BUFFER_PAGE_COUNT) *
                               (TRC_CFG_PAGED_EVENT_BUFFER_PAGE_SIZE);
...
void vTraceStoreISRBegin(traceHandle handle) {
  ...
  if (ISR_stack_index == -1) isPendingContextSwitch = 0;
...
#if (TRC_CFG_INCLUDE_ISR_TRACING == 1)
      prvTraceStoreEvent1(PSF_EVENT_TS_RESUME,
  ...
  switch (errCode) {
    ...
    */
    ...
    case PSF_WARNING_STREAM_PORT_INITIAL_BLOCKING:
      ...
      is
      smaller than the initial transmission (trace header, object table, and
      symbol table). */

      return "Blocking in vTraceEnable (see xTraceGetLastError)";

    case PSF_ERROR_EVENT_CODE_TOO_LARGE:
      /* The highest allowed event code is 4095, anything higher is an
      unexpected error. Please contact support@percepio.com for assistance.*/

      return "Invalid event code (see prvTraceGetError)";

    case PSF_ERROR_ISR_NESTING_OVERFLOW:
      /* Nesting of ISR trace calls exceeded the limit
      (TRC_CFG_MAX_ISR_NESTING). If this is unlikely, make sure that you call
      vTraceStoreISRExit in the end of all ISR handlers. Or increase
    ...
    case PSF_ERROR_TZCTRLTASK_NOT_CREATED:
      /* vTraceEnable failed creating the trace control task (TzCtrl) -
      incorrect parameters (priority?) or insufficient heap size? */
...
int prvIsValidCommand(TracealyzerCommandType* cmd) {
  ...
  if (cmd->checksumMSB != (unsigned char)(checksum >> 8)) return 0;

  if (cmd->checksumLSB != (unsigned char)(checksum & 0xFF)) return 0;
...
static int prvAllocateBufferPage(int prevPage) {
  ...
  while ((PageInfo[index].Status != PAGE_STATUS_FREE) &&
         (count++ < (TRC_CFG_PAGED_EVENT_BUFFER_PAGE_COUNT))) {
...
static void prvPageReadComplete(int pageIndex) {
  ...
  PageInfo[pageIndex].WritePointer =
      &EventBuffer[pageIndex * (TRC_CFG_PAGED_EVENT_BUFFER_PAGE_SIZE)];
...
static int prvGetBufferPage(int32_t* bytesUsed) {
  ...
  int count = 0;
  int8_t index =
      (int8_t)((lastPage + 1) % (TRC_CFG_PAGED_EVENT_BUFFER_PAGE_COUNT));

  while ((PageInfo[index].Status != PAGE_STATUS_READ) &&
         (count++ < (TRC_CFG_PAGED_EVENT_BUFFER_PAGE_COUNT))) {
  ...
  if (PageInfo[index].Status == PAGE_STATUS_READ) {
    *bytesUsed =
        (TRC_CFG_PAGED_EVENT_BUFFER_PAGE_SIZE)-PageInfo[index].BytesRemaining;
    lastPage = index;
    return index;
  }
...
uint32_t prvPagedEventBufferTransfer(void) {
  ...
  /* bytesToTransfer now contains the number of "valid" bytes in the buffer
  page, that should be transmitted.
    ...
    while (1) /* Keep going until we have transferred all that we intended to */
    {
      if (TRC_STREAM_PORT_WRITE_DATA(
        ...
        if (bytesTransferredTotal == bytesToTransfer) {
          /* All bytes have been transferred. Mark the buffer page as "Read
           * Complete" (so it can be written to) and return OK. */
...
void* prvPagedEventBufferGetWritePointer(int sizeOfEvent) {
  ...
  if (PageInfo[currentWritePage].BytesRemaining - sizeOfEvent < 0) {
    ...
    TotalBytesRemaining -=
        PageInfo[currentWritePage].BytesRemaining;  // Last trailing bytes
  ...
  PageInfo[currentWritePage].BytesRemaining =
      (uint16_t)(PageInfo[currentWritePage].BytesRemaining - sizeOfEvent);
...
void prvPagedEventBufferInit(char* buffer) {
  ...
  for (i = 0; i < (TRC_CFG_PAGED_EVENT_BUFFER_PAGE_COUNT); i++) {
    ...
    PageInfo[i].WritePointer =
        &EventBuffer[i * (TRC_CFG_PAGED_EVENT_BUFFER_PAGE_SIZE)];
