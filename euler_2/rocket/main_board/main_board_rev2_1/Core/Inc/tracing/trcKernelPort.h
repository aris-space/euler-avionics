...
/* Legacy FreeRTOS version codes for backwards compatibility with old trace
 * configurations */
...
#define prvGetStreamBufferType(x) \
  (((StreamBuffer_t*)x)->ucFlags & sbFLAGS_IS_MESSAGE_BUFFER)
#else
#define prvGetStreamBufferType(x) 0
#endif

/* Added mainly for our internal testing. This makes it easier to create test
   applications that runs on multiple FreeRTOS versions. */
...
#if (defined(TRC_USE_TRACEALYZER_RECORDER)) && \
...
/*******************************************************************************
 * INCLUDE_xTaskGetCurrentTaskHandle must be set to 1 for tracing to work
 *properly
 ******************************************************************************/
...
#else /* defined (TRC_CFG_ENABLE_STACK_MONITOR) && \
         (TRC_CFG_ENABLE_STACK_MONITOR == 1) */
...
#define INCLUDE_xTaskGetSchedulerState 1
          ...
          (TRC_CFG_INCLUDE_ISR_TRACING == 1)) || (TRC_CFG_RECORDER_MODE == \
          TRC_RECORDER_MODE_STREAMING)) */

#define TRACE_KERNEL_VERSION 0x1AA1
#define TRACE_TICK_RATE_HZ   configTICK_RATE_HZ /* Defined in "FreeRTOS.h" */
#define TRACE_CPU_CLOCK_HZ \
  configCPU_CLOCK_HZ /* Defined in "FreeRTOSConfig.h" */
...
#define TRACE_ENTER_CRITICAL_SECTION() \
  {                                    \
    __irq_status = __get_PRIMASK();    \
    __set_PRIMASK(1);                  \
  } /* PRIMASK disables ALL interrupts - allows for tracing in any ISR */
#define TRACE_EXIT_CRITICAL_SECTION() \
  { __set_PRIMASK(__irq_status); }
#endif

#if (TRC_CFG_HARDWARE_PORT == TRC_HARDWARE_PORT_ARM_CORTEX_A9) || \
    (TRC_CFG_HARDWARE_PORT == TRC_HARDWARE_PORT_XILINX_ZyncUltraScaleR5)
...
#define TRACE_ENTER_CRITICAL_SECTION() \
  { __irq_mask_status = cortex_a9_r5_enter_critical(); }

#define TRACE_EXIT_CRITICAL_SECTION() \
  { cortex_a9_r5_exit_critical(__irq_mask_status); }

#endif

#if ((TRC_CFG_HARDWARE_PORT == TRC_HARDWARE_PORT_Renesas_RX600) || \
     (TRC_CFG_HARDWARE_PORT == TRC_HARDWARE_PORT_MICROCHIP_PIC24_PIC32))
#define TRACE_ALLOC_CRITICAL_SECTION() int __irq_status;
#define TRACE_ENTER_CRITICAL_SECTION() \
  { __irq_status = portSET_INTERRUPT_MASK_FROM_ISR(); }
#define TRACE_EXIT_CRITICAL_SECTION() \
  { portCLEAR_INTERRUPT_MASK_FROM_ISR(__irq_status); }
#endif
...
#define TRACE_ENTER_CRITICAL_SECTION() \
  { __irq_status = alt_irq_disable_all(); }
#define TRACE_EXIT_CRITICAL_SECTION() \
  { alt_irq_enable_all(__irq_status); }
#endif

#if (TRC_CFG_HARDWARE_PORT == TRC_HARDWARE_PORT_Win32)
/* In the Win32 port, there are no real interrupts, so we can use the normal
 * critical sections */
 ...
 ******************************************************************************/
#define isQueueReceiveHookActuallyPeek \
  (__LINE__ > 1674) /* Half way between the closes trace points */
...
#define isQueueReceiveHookActuallyPeek \
  (__LINE__ <                          \
   0) /* instead of pdFALSE to fix a warning of "constant condition" */
...
#endif /* (TRC_CFG_INCLUDE_TIMER_EVENTS == 1 && TRC_CFG_FREERTOS_VERSION >= \
          TRC_FREERTOS_VERSION_10_0_0) */
...
#endif /* (TRC_CFG_INCLUDE_EVENT_GROUP_EVENTS == 1 && TRC_CFG_FREERTOS_VERSION \
          >= TRC_FREERTOS_VERSION_10_0_0) */
...
#define TRACE_SET_TASK_FILTER(pxTask, group) \
  prvTraceSetTaskNumberHigh16((void*)pxTask, group)

#define TRACE_GET_QUEUE_FILTER(pxObject) \
  prvTraceGetQueueNumberHigh16((void*)pxObject)
#define TRACE_SET_QUEUE_FILTER(pxObject, group) \
  prvTraceSetQueueNumberHigh16((void*)pxObject, group)

#if (TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_10_0_0)
#define TRACE_GET_EVENTGROUP_FILTER(pxObject) \
  prvTraceGetEventGroupNumberHigh16((void*)pxObject)
#define TRACE_SET_EVENTGROUP_FILTER(pxObject, group) \
  prvTraceSetEventGroupNumberHigh16((void*)pxObject, group)
...
#if (TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_10_0_0)
#define TRACE_GET_TIMER_FILTER(pxObject) \
  prvTraceGetTimerNumberHigh16((void*)pxObject)
#define TRACE_SET_TIMER_FILTER(pxObject, group) \
  prvTraceSetTimerNumberHigh16((void*)pxObject, group)
#else /* (TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_10_0_0) */
...
#define TRACE_GET_STREAMBUFFER_FILTER(pxObject) \
  prvTraceGetStreamBufferNumberHigh16((void*)pxObject)
#define TRACE_SET_STREAMBUFFER_FILTER(pxObject, group) \
  prvTraceSetStreamBufferNumberHigh16((void*)pxObject, group)

/* We can only support filtering if FreeRTOS is at least v8.0 */
#if (TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_8_X_X)
#define TRACE_GET_OBJECT_FILTER(CLASS, pxObject) \
  TRACE_GET_##CLASS##_FILTER(pxObject)
#define TRACE_SET_OBJECT_FILTER(CLASS, pxObject, group) \
  TRACE_SET_##CLASS##_FILTER(pxObject, group)
...
/* Expands name if this header is included... uxQueueType must be a macro that
 * only exists in queue.c or whatever, and it must expand to nothing or to
 * something that's valid in identifiers */
#define xQueueGiveFromISR(a, b) TZ__CAT(xQueueGiveFromISR__, uxQueueType)(a, b)

/* If in queue.c, the "uxQueueType" macro expands to "pcHead". queueSEND_TO_BACK
 * is the value we need to send in */
#define xQueueGiveFromISR__pcHead(__a, __b)                               \
  MyWrapper_xQueueGiveFromISR(__a, __b, const BaseType_t xCopyPosition);  \
  BaseType_t xQueueGiveFromISR(__a, __b) {                                \
    return MyWrapper_xQueueGiveFromISR(xQueue, pxHigherPriorityTaskWoken, \
                                       queueSEND_TO_BACK);                \
  }                                                                       \
  BaseType_t MyWrapper_xQueueGiveFromISR(__a, __b,                        \
...
#define TRACE_KERNEL_OBJECT_COUNT                               \
  ((TRC_CFG_NQUEUE) + (TRC_CFG_NSEMAPHORE) + (TRC_CFG_NMUTEX) + \
   (TRC_CFG_NTASK) + (TRC_CFG_NISR) + (TRC_CFG_NTIMER) +        \
   (TRC_CFG_NEVENTGROUP) + (TRC_CFG_NSTREAMBUFFER) + (TRC_CFG_NMESSAGEBUFFER))
...
/* Task properties (except name):	Byte 0: Current priority
                                                                        Byte 1:
   state (if already active) Byte 2: legacy, not used Byte 3: legacy, not used
 */
#define PropertyTableSizeTask ((TRC_CFG_NAME_LEN_TASK) + 4)

/* ISR properties:					Byte 0: priority
                                                                        Byte 1:
   state (if already active) */
#define PropertyTableSizeISR ((TRC_CFG_NAME_LEN_ISR) + 2)

/* TRC_CFG_NTIMER properties:				Byte 0: state (unused
 * for now) */
#define PropertyTableSizeTimer ((TRC_CFG_NAME_LEN_TIMER) + 1)

/* TRC_CFG_NEVENTGROUP properties:			Byte 0-3: state (unused
 * for now)*/
...
#define StartIndexSemaphore \
  (StartIndexQueue + (TRC_CFG_NQUEUE)*PropertyTableSizeQueue)
#define StartIndexMutex \
  (StartIndexSemaphore + (TRC_CFG_NSEMAPHORE)*PropertyTableSizeSemaphore)
#define StartIndexTask \
  (StartIndexMutex + (TRC_CFG_NMUTEX)*PropertyTableSizeMutex)
...
#define StartIndexEventGroup \
  (StartIndexTimer + (TRC_CFG_NTIMER)*PropertyTableSizeTimer)
#define StartIndexStreamBuffer \
  (StartIndexEventGroup + (TRC_CFG_NEVENTGROUP)*PropertyTableSizeEventGroup)
#define StartIndexMessageBuffer \
  (StartIndexStreamBuffer +     \
   (TRC_CFG_NSTREAMBUFFER)*PropertyTableSizeStreamBuffer)

/* Number of bytes used by the object table */
#define TRACE_OBJECT_TABLE_SIZE \
  (StartIndexMessageBuffer +    \
   (TRC_CFG_NMESSAGEBUFFER)*PropertyTableSizeMessageBuffer)
 ...
 ******************************************************************************/
#define EVENTGROUP_OBJCLOSE_PROP_TRCSUCCESS \
  (EVENTGROUP_OBJCLOSE_NAME_TRCSUCCESS + 8UL) /*0x10*/
 ...
 ******************************************************************************/
#define EVENTGROUP_CREATE_OBJ_TRCSUCCESS \
  (EVENTGROUP_OBJCLOSE_PROP_TRCSUCCESS + 8UL) /*0x18*/
 ...
 * or mutex).
 ******************************************************************************/
#define EVENTGROUP_SEND_TRCSUCCESS \
  (EVENTGROUP_CREATE_OBJ_TRCSUCCESS + 8UL) /*0x20*/
 ...
 * or mutex).
 ******************************************************************************/
#define EVENTGROUP_RECEIVE_TRCSUCCESS \
  (EVENTGROUP_SEND_TRCSUCCESS + 8UL) /*0x28*/
...
#define EVENTGROUP_SEND_TRCFAILED \
  (EVENTGROUP_CREATE_OBJ_TRCFAILED + 8UL) /*0x48*/
...
#define EVENTGROUP_SEND_FROM_ISR_TRCFAILED \
  (EVENTGROUP_RECEIVE_TRCFAILED + 8UL) /*0x58*/
...
#define EVENTGROUP_DELETE_OBJ_TRCSUCCESS \
  (EVENTGROUP_PEEK_TRCSUCCESS + 8UL) /*0x80*/
...
#define PEND_FUNC_CALL_FROM_ISR_TRCFAILED \
  (EVENTGROUP_MISC_PLACEHOLDER + 3UL) /*0x93*/
 ...
 * which means a limit of 0xFF (255UL). The XTS16 is used when the original
 *event has a 16 bit DTS field and thereby can handle values up to 0xFFFF
 *(65535UL).
 *
...
#define TRACE_TASK_NOTIFY_TAKE_TRCFAILED \
  (TRACE_TASK_NOTIFY_GROUP + 3UL) /*0xD5*/
...
#define TRACE_TASK_NOTIFY_WAIT_TRCFAILED \
  (TRACE_TASK_NOTIFY_GROUP + 6UL) /*0xD8*/
...
#define TRACE_STREAMBUFFER_OBJCLOSE_NAME_TRCSUCCESS \
  (EVENTGROUP_STREAMBUFFER_DIV + 2UL) /*0xE4*/
#define TRACE_MESSAGEBUFFER_OBJCLOSE_NAME_TRCSUCCESS \
  (EVENTGROUP_STREAMBUFFER_DIV + 3UL) /*0xE5*/
#define TRACE_STREAMBUFFER_OBJCLOSE_PROP_TRCSUCCESS \
  (EVENTGROUP_STREAMBUFFER_DIV + 4UL) /*0xE6*/
#define TRACE_MESSAGEBUFFER_OBJCLOSE_PROP_TRCSUCCESS \
  (EVENTGROUP_STREAMBUFFER_DIV + 5UL) /*0xE7*/
...
#define TRACE_STREAMBUFFER_CREATE_OBJ_TRCSUCCESS \
  (EVENTGROUP_CREATE_OBJ_TRCSUCCESS + 4UL) /*0x1C*/
#define TRACE_STREAMBUFFER_CREATE_OBJ_TRCFAILED \
  (EVENTGROUP_CREATE_OBJ_TRCFAILED + 4UL) /*0x44*/
#define TRACE_STREAMBUFFER_DELETE_OBJ_TRCSUCCESS \
  (EVENTGROUP_DELETE_OBJ_TRCSUCCESS + 4UL) /*0x84*/
#define TRACE_STREAMBUFFER_SEND_TRCSUCCESS \
  (EVENTGROUP_SEND_TRCSUCCESS + 3UL) /*0x23*/
#define TRACE_STREAMBUFFER_SEND_TRCBLOCK \
  (EVENTGROUP_SEND_TRCBLOCK + 3UL) /*0x73*/
#define TRACE_STREAMBUFFER_SEND_TRCFAILED \
  (EVENTGROUP_SEND_TRCFAILED + 3UL) /*0x4B*/
#define TRACE_STREAMBUFFER_RECEIVE_TRCSUCCESS \
  (EVENTGROUP_RECEIVE_TRCSUCCESS + 3UL) /*0x2B*/
#define TRACE_STREAMBUFFER_RECEIVE_TRCBLOCK \
  (EVENTGROUP_RECEIVE_TRCBLOCK + 3UL) /*0x6B*/
#define TRACE_STREAMBUFFER_RECEIVE_TRCFAILED \
  (EVENTGROUP_RECEIVE_TRCFAILED + 3UL) /*0x53*/
#define TRACE_STREAMBUFFER_SEND_FROM_ISR_TRCSUCCESS \
  (EVENTGROUP_SEND_FROM_ISR_TRCSUCCESS + 3UL) /*0x33*/
#define TRACE_STREAMBUFFER_SEND_FROM_ISR_TRCFAILED \
  (EVENTGROUP_SEND_FROM_ISR_TRCFAILED + 3UL) /*0x5B*/
#define TRACE_STREAMBUFFER_RECEIVE_FROM_ISR_TRCSUCCESS \
  (EVENTGROUP_RECEIVE_FROM_ISR_TRCSUCCESS + 3UL) /*0x3B*/
#define TRACE_STREAMBUFFER_RECEIVE_FROM_ISR_TRCFAILED \
  (EVENTGROUP_RECEIVE_FROM_ISR_TRCFAILED + 3UL) /*0x63*/

/* The following are using previously "lost" event codes. These macros aren't
 * even directly referenced, instead we do (equivalent STREAMBUFFER code) + 1.
 */
#define TRACE_MESSAGEBUFFER_CREATE_OBJ_TRCSUCCESS \
  (EVENTGROUP_CREATE_OBJ_TRCSUCCESS + 5UL) /*0x1D*/
#define TRACE_MESSAGEBUFFER_CREATE_OBJ_TRCFAILED \
  (EVENTGROUP_CREATE_OBJ_TRCFAILED + 5UL) /*0x45*/
#define TRACE_MESSAGEBUFFER_DELETE_OBJ_TRCSUCCESS \
  (EVENTGROUP_DELETE_OBJ_TRCSUCCESS + 5UL) /*0x85*/
#define TRACE_MESSAGEBUFFER_SEND_TRCSUCCESS \
  (EVENTGROUP_SEND_TRCSUCCESS + 4UL) /*0x24*/
#define TRACE_MESSAGEBUFFER_SEND_TRCBLOCK \
  (EVENTGROUP_SEND_TRCBLOCK + 4UL) /*0x74*/
#define TRACE_MESSAGEBUFFER_SEND_TRCFAILED \
  (EVENTGROUP_SEND_TRCFAILED + 4UL) /*0x4C*/
#define TRACE_MESSAGEBUFFER_RECEIVE_TRCSUCCESS \
  (EVENTGROUP_RECEIVE_TRCSUCCESS + 4UL) /*0x2C*/
#define TRACE_MESSAGEBUFFER_RECEIVE_TRCBLOCK \
  (EVENTGROUP_RECEIVE_TRCBLOCK + 4UL) /*0x6C*/
#define TRACE_MESSAGEBUFFER_RECEIVE_TRCFAILED \
  (EVENTGROUP_RECEIVE_TRCFAILED + 4UL) /*0x54*/
#define TRACE_MESSAGEBUFFER_SEND_FROM_ISR_TRCSUCCESS \
  (EVENTGROUP_SEND_FROM_ISR_TRCSUCCESS + 4UL) /*0x34*/
#define TRACE_MESSAGEBUFFER_SEND_FROM_ISR_TRCFAILED \
  (EVENTGROUP_SEND_FROM_ISR_TRCFAILED + 4UL) /*0x5C*/
#define TRACE_MESSAGEBUFFER_RECEIVE_FROM_ISR_TRCSUCCESS \
  (EVENTGROUP_RECEIVE_FROM_ISR_TRCSUCCESS + 4UL) /*0x3C*/
#define TRACE_MESSAGEBUFFER_RECEIVE_FROM_ISR_TRCFAILED \
  (EVENTGROUP_RECEIVE_FROM_ISR_TRCFAILED + 4UL) /*0x64*/

#define TRACE_QUEUE_SEND_TO_FRONT_TRCSUCCESS \
  (EVENTGROUP_SEND_TRCSUCCESS + 5UL) /*0x25*/
#define TRACE_QUEUE_SEND_TO_FRONT_TRCBLOCK \
  (EVENTGROUP_SEND_TRCBLOCK + 5UL) /*0x75*/
#define TRACE_QUEUE_SEND_TO_FRONT_TRCFAILED \
  (EVENTGROUP_SEND_TRCFAILED + 5UL) /*0x4D*/
#define TRACE_QUEUE_SEND_TO_FRONT_FROM_ISR_TRCSUCCESS \
  (EVENTGROUP_SEND_FROM_ISR_TRCSUCCESS + 5UL) /*0x35*/
#define TRACE_QUEUE_SEND_TO_FRONT_FROM_ISR_TRCFAILED \
  (EVENTGROUP_SEND_FROM_ISR_TRCFAILED + 5UL) /*0x5D*/
...
#define TRACE_GET_TRACE_CLASS_FROM_QUEUE_CLASS(kernelClass) \
  TraceQueueClassTable[kernelClass]
#define TRACE_GET_TRACE_CLASS_FROM_QUEUE_OBJECT(pxObject) \
  TRACE_GET_TRACE_CLASS_FROM_QUEUE_CLASS(prvTraceGetQueueType(pxObject))
...
#define TRACE_GET_TRACE_CLASS_FROM_EVENTGROUP_CLASS(kernelClass) \
  (TRACE_CLASS_EVENTGROUP)
#define TRACE_GET_TRACE_CLASS_FROM_EVENTGROUP_OBJECT(pxObject) \
  (TRACE_CLASS_EVENTGROUP)

/* TRACE_GET_TRACE_CLASS_FROM_STREAMBUFFER_CLASS can only be accessed with a
 * parameter indicating if it is a MessageBuffer */
#define TRACE_GET_TRACE_CLASS_FROM_STREAMBUFFER_CLASS(xIsMessageBuffer) \
  (xIsMessageBuffer == 1 ? TRACE_CLASS_MESSAGEBUFFER : TRACE_CLASS_STREAMBUFFER)
#define TRACE_GET_TRACE_CLASS_FROM_STREAMBUFFER_OBJECT(pxObject)     \
  (prvGetStreamBufferType(pxObject) == 1 ? TRACE_CLASS_MESSAGEBUFFER \
                                         : TRACE_CLASS_STREAMBUFFER)

/* Generic versions */
#define TRACE_GET_CLASS_TRACE_CLASS(CLASS, kernelClass) \
  TRACE_GET_TRACE_CLASS_FROM_##CLASS##_CLASS(kernelClass)
#define TRACE_GET_OBJECT_TRACE_CLASS(CLASS, pxObject) \
  TRACE_GET_TRACE_CLASS_FROM_##CLASS##_OBJECT(pxObject)

/******************************
 * MACROS TO GET OBJECT NUMBER *
 ******************************/
#define TRACE_GET_TASK_NUMBER(pxTCB) \
  (traceHandle)(prvTraceGetTaskNumberLow16(pxTCB))
#define TRACE_SET_TASK_NUMBER(pxTCB) \
  prvTraceSetTaskNumberLow16(        \
      pxTCB,                         \
      prvTraceGetObjectHandle(TRACE_GET_OBJECT_TRACE_CLASS(TASK, pxTCB)));

#define TRACE_GET_QUEUE_NUMBER(queue) \
  ((traceHandle)prvTraceGetQueueNumberLow16(queue))
#define TRACE_SET_QUEUE_NUMBER(queue)           \
  prvTraceSetQueueNumberLow16(                  \
      queue, (uint16_t)prvTraceGetObjectHandle( \
                 TRACE_GET_OBJECT_TRACE_CLASS(QUEUE, queue)));

#if (TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_10_0_0)
#define TRACE_GET_TIMER_NUMBER(tmr) \
  ((traceHandle)prvTraceGetTimerNumberLow16(tmr))
#define TRACE_SET_TIMER_NUMBER(tmr)                              \
  prvTraceSetTimerNumberLow16(tmr,                               \
                              (uint16_t)prvTraceGetObjectHandle( \
                                  TRACE_GET_OBJECT_TRACE_CLASS(TIMER, tmr)));
#else /* (TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_10_0_0) */
#define TRACE_GET_TIMER_NUMBER(tmr) \
  ((traceHandle)((Timer_t*)tmr)->uxTimerNumber)
#define TRACE_SET_TIMER_NUMBER(tmr) \
  ((Timer_t*)tmr)->uxTimerNumber =  \
      prvTraceGetObjectHandle(TRACE_GET_OBJECT_TRACE_CLASS(TIMER, tmr));
#endif /* (TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_10_0_0) */

#if (TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_10_0_0)
#define TRACE_GET_EVENTGROUP_NUMBER(eg) \
  ((traceHandle)prvTraceGetEventGroupNumberLow16(eg))
#define TRACE_SET_EVENTGROUP_NUMBER(eg)      \
  prvTraceSetEventGroupNumberLow16(          \
      eg, (uint16_t)prvTraceGetObjectHandle( \
              TRACE_GET_OBJECT_TRACE_CLASS(EVENTGROUP, eg)));
#else /* (TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_10_0_0) */
#define TRACE_GET_EVENTGROUP_NUMBER(eg) ((traceHandle)uxEventGroupGetNumber(eg))
#define TRACE_SET_EVENTGROUP_NUMBER(eg)     \
  ((EventGroup_t*)eg)->uxEventGroupNumber = \
      prvTraceGetObjectHandle(TRACE_GET_OBJECT_TRACE_CLASS(EVENTGROUP, eg));
#endif /* (TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_10_0_0) */

#define TRACE_GET_STREAMBUFFER_NUMBER(sb) \
  ((traceHandle)prvTraceGetStreamBufferNumberLow16(sb))
#define TRACE_SET_STREAMBUFFER_NUMBER(sb)    \
  prvTraceSetStreamBufferNumberLow16(        \
      sb, (uint16_t)prvTraceGetObjectHandle( \
              TRACE_GET_OBJECT_TRACE_CLASS(STREAMBUFFER, sb)));

/* Generic versions */
#define TRACE_GET_OBJECT_NUMBER(CLASS, pxObject) \
  TRACE_GET_##CLASS##_NUMBER(pxObject)
#define TRACE_SET_OBJECT_NUMBER(CLASS, pxObject) \
  TRACE_SET_##CLASS##_NUMBER(pxObject)

/******************************
 * MACROS TO GET EVENT CODES   *
 ******************************/
#define TRACE_GET_TASK_CLASS_EVENT_CODE(SERVICE, RESULT, kernelClass) \
  (uint8_t)(EVENTGROUP_##SERVICE##_##RESULT +                         \
            TRACE_GET_CLASS_TRACE_CLASS(TASK, kernelClass))
#define TRACE_GET_QUEUE_CLASS_EVENT_CODE(SERVICE, RESULT, kernelClass) \
  (uint8_t)(EVENTGROUP_##SERVICE##_##RESULT +                          \
            TRACE_GET_CLASS_TRACE_CLASS(QUEUE, kernelClass))
#define TRACE_GET_TIMER_CLASS_EVENT_CODE(SERVICE, RESULT, kernelClass) \
  --THIS IS NOT USED--
#define TRACE_GET_EVENTGROUP_CLASS_EVENT_CODE(SERVICE, RESULT, kernelClass) \
  --THIS IS NOT USED--
#define TRACE_GET_STREAMBUFFER_CLASS_EVENT_CODE(SERVICE, RESULT, \
                                                isMessageBuffer) \
  (uint8_t)(TRACE_STREAMBUFFER_##SERVICE##_##RESULT + (uint8_t)isMessageBuffer)

#define TRACE_GET_TASK_OBJECT_EVENT_CODE(SERVICE, RESULT, pxTCB) \
  (uint8_t)(EVENTGROUP_##SERVICE##_##RESULT + TRACE_CLASS_TASK)
#define TRACE_GET_QUEUE_OBJECT_EVENT_CODE(SERVICE, RESULT, pxObject) \
  (uint8_t)(EVENTGROUP_##SERVICE##_##RESULT +                        \
            TRACE_GET_OBJECT_TRACE_CLASS(QUEUE, pxObject))
#define TRACE_GET_TIMER_OBJECT_EVENT_CODE(SERVICE, RESULT, UNUSED) \
  --THIS IS NOT USED--
#define TRACE_GET_EVENTGROUP_OBJECT_EVENT_CODE(SERVICE, RESULT, UNUSED) \
  --THIS IS NOT USED--
#define TRACE_GET_STREAMBUFFER_OBJECT_EVENT_CODE(SERVICE, RESULT, pxObject) \
  (uint8_t)(TRACE_STREAMBUFFER_##SERVICE##_##RESULT +                       \
            prvGetStreamBufferType(pxObject))

/* Generic versions */
#define TRACE_GET_CLASS_EVENT_CODE(SERVICE, RESULT, CLASS, kernelClass) \
  TRACE_GET_##CLASS##_CLASS_EVENT_CODE(SERVICE, RESULT, kernelClass)
#define TRACE_GET_OBJECT_EVENT_CODE(SERVICE, RESULT, CLASS, pxObject) \
  TRACE_GET_##CLASS##_OBJECT_EVENT_CODE(SERVICE, RESULT, pxObject)
...
/* Called on each OS tick. Will call uiPortGetTimestamp to make sure it is
 * called at least once every OS tick. */
#undef traceTASK_INCREMENT_TICK

#if (TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_10_3_0)

#define traceTASK_INCREMENT_TICK(xTickCount)                     \
  if (uxSchedulerSuspended == (unsigned portBASE_TYPE)pdTRUE ||  \
      xPendedTicks == 0) {                                       \
    trcKERNEL_HOOKS_INCREMENT_TICK();                            \
  }                                                              \
  if (uxSchedulerSuspended == (unsigned portBASE_TYPE)pdFALSE) { \
    trcKERNEL_HOOKS_NEW_TIME(DIV_NEW_TIME, xTickCount + 1);      \
  }
...
#define traceTASK_CREATE(pxNewTCB)                                           \
  if (pxNewTCB != NULL) {                                                    \
    trcKERNEL_HOOKS_TASK_CREATE(                                             \
        TRACE_GET_OBJECT_EVENT_CODE(CREATE_OBJ, TRCSUCCESS, TASK, pxNewTCB), \
        TASK, pxNewTCB);                                                     \
    prvAddTaskToStackMonitor(pxNewTCB);                                      \
  }

/* Called in vTaskCreate, if it fails (typically if the stack can not be
 * allocated) */
#undef traceTASK_CREATE_FAILED
#define traceTASK_CREATE_FAILED()                         \
  trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_NUMERIC_PARAM_ONLY( \
      TRACE_GET_CLASS_EVENT_CODE(CREATE_OBJ, TRCFAILED, TASK, NOT_USED), 0);
...
#define traceTASK_DELETE(pxTaskToDelete)                             \
  {                                                                  \
    ...
    trcKERNEL_HOOKS_TASK_DELETE(                                     \
        TRACE_GET_OBJECT_EVENT_CODE(DELETE_OBJ, TRCSUCCESS, TASK,    \
                                    pxTaskToDelete),                 \
        TRACE_GET_OBJECT_EVENT_CODE(OBJCLOSE_NAME, TRCSUCCESS, TASK, \
                                    pxTaskToDelete),                 \
        TRACE_GET_OBJECT_EVENT_CODE(OBJCLOSE_PROP, TRCSUCCESS, TASK, \
                                    pxTaskToDelete),                 \
        pxTaskToDelete);                                             \
...
/* Called in xQueueCreate, and thereby for all other object based on queues,
 * such as semaphores. */
#undef traceQUEUE_CREATE
#define traceQUEUE_CREATE(pxNewQueue)                                         \
  trcKERNEL_HOOKS_OBJECT_CREATE(                                              \
      TRACE_GET_OBJECT_EVENT_CODE(CREATE_OBJ, TRCSUCCESS, QUEUE, pxNewQueue), \
      QUEUE, pxNewQueue);
...
#define traceQUEUE_CREATE_FAILED(queueType)               \
  trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_NUMERIC_PARAM_ONLY( \
      TRACE_GET_CLASS_EVENT_CODE(CREATE_OBJ, TRCFAILED, QUEUE, queueType), 0);
...
#define traceQUEUE_DELETE(pxQueue)                                           \
  {                                                                          \
    ...
    trcKERNEL_HOOKS_OBJECT_DELETE(                                           \
        TRACE_GET_OBJECT_EVENT_CODE(DELETE_OBJ, TRCSUCCESS, QUEUE, pxQueue), \
        TRACE_GET_OBJECT_EVENT_CODE(OBJCLOSE_NAME, TRCSUCCESS, QUEUE,        \
                                    pxQueue),                                \
        TRACE_GET_OBJECT_EVENT_CODE(OBJCLOSE_PROP, TRCSUCCESS, QUEUE,        \
                                    pxQueue),                                \
        QUEUE, pxQueue);                                                     \
    TRACE_EXIT_CRITICAL_SECTION();                                           \
  }
...
#if (TRC_CFG_FREERTOS_VERSION < TRC_FREERTOS_VERSION_9_0_0)
/* Called in xQueueCreateMutex, and thereby also from xSemaphoreCreateMutex and
...
/* Called in xQueueCreateMutex when the operation fails (when memory allocation
 * fails) */
#undef traceCREATE_MUTEX_FAILED
#define traceCREATE_MUTEX_FAILED()                             \
  trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_NUMERIC_PARAM_ONLY(      \
      TRACE_GET_CLASS_EVENT_CODE(CREATE_OBJ, TRCFAILED, QUEUE, \
...
#define traceGIVE_MUTEX_RECURSIVE_FAILED(pxMutex)                          \
  trcKERNEL_HOOKS_KERNEL_SERVICE(                                          \
      TRACE_GET_OBJECT_EVENT_CODE(SEND, TRCFAILED, QUEUE, pxMutex), QUEUE, \
      pxMutex);
...
#define traceQUEUE_SEND(pxQueue)                                            \
  trcKERNEL_HOOKS_KERNEL_SERVICE(                                           \
      xCopyPosition == queueSEND_TO_BACK                                    \
          ? (TRACE_GET_OBJECT_EVENT_CODE(SEND, TRCSUCCESS, QUEUE, pxQueue)) \
          : TRACE_QUEUE_SEND_TO_FRONT_TRCSUCCESS,                           \
      QUEUE, pxQueue);                                                      \
  trcKERNEL_HOOKS_SET_OBJECT_STATE(                                         \
      QUEUE, pxQueue,                                                       \
      TRACE_GET_OBJECT_TRACE_CLASS(QUEUE, pxQueue) == TRACE_CLASS_MUTEX     \
          ? (uint8_t)0                                                      \
          : (uint8_t)(pxQueue->uxMessagesWaiting + 1));

/* Called when a message is sent to a queue set */
#undef traceQUEUE_SET_SEND
#define traceQUEUE_SET_SEND(pxQueue)                                        \
  trcKERNEL_HOOKS_KERNEL_SERVICE(                                           \
      TRACE_GET_OBJECT_EVENT_CODE(SEND, TRCSUCCESS, QUEUE, pxQueue), QUEUE, \
      pxQueue);                                                             \
  trcKERNEL_HOOKS_SET_OBJECT_STATE(QUEUE, pxQueue,                          \
                                   (uint8_t)(pxQueue->uxMessagesWaiting + 1));
...
#define traceQUEUE_SEND_FAILED(pxQueue)                                    \
  trcKERNEL_HOOKS_KERNEL_SERVICE(                                          \
      xCopyPosition == queueSEND_TO_BACK                                   \
          ? (TRACE_GET_OBJECT_EVENT_CODE(SEND, TRCFAILED, QUEUE, pxQueue)) \
          : TRACE_QUEUE_SEND_TO_FRONT_TRCFAILED,                           \
      QUEUE, pxQueue);
...
#define traceBLOCKING_ON_QUEUE_SEND(pxQueue)                              \
  trcKERNEL_HOOKS_KERNEL_SERVICE(                                         \
      xCopyPosition == queueSEND_TO_BACK                                  \
          ? (TRACE_GET_OBJECT_EVENT_CODE(SEND, TRCBLOCK, QUEUE, pxQueue)) \
          : TRACE_QUEUE_SEND_TO_FRONT_TRCBLOCK,                           \
      QUEUE, pxQueue);
...
#define traceQUEUE_RECEIVE(pxQueue)                                           \
  if (isQueueReceiveHookActuallyPeek) {                                       \
    trcKERNEL_HOOKS_KERNEL_SERVICE(                                           \
        TRACE_GET_OBJECT_EVENT_CODE(PEEK, TRCSUCCESS, QUEUE, pxQueue), QUEUE, \
        pxQueue);                                                             \
  } else {                                                                    \
    trcKERNEL_HOOKS_KERNEL_SERVICE(                                           \
        TRACE_GET_OBJECT_EVENT_CODE(RECEIVE, TRCSUCCESS, QUEUE, pxQueue),     \
        QUEUE, pxQueue);                                                      \
  }                                                                           \
  trcKERNEL_HOOKS_SET_OBJECT_STATE(                                           \
      QUEUE, pxQueue,                                                         \
      TRACE_GET_OBJECT_TRACE_CLASS(QUEUE, pxQueue) == TRACE_CLASS_MUTEX       \
          ? (uint8_t)TRACE_GET_TASK_NUMBER(TRACE_GET_CURRENT_TASK())          \
          : (uint8_t)(pxQueue->uxMessagesWaiting - 1));
...
#define traceQUEUE_RECEIVE_FAILED(pxQueue)                                   \
  if (isQueueReceiveHookActuallyPeek) {                                      \
    trcKERNEL_HOOKS_KERNEL_SERVICE(                                          \
        TRACE_GET_OBJECT_EVENT_CODE(PEEK, TRCFAILED, QUEUE, pxQueue), QUEUE, \
        pxQueue);                                                            \
  } else {                                                                   \
    trcKERNEL_HOOKS_KERNEL_SERVICE(                                          \
        TRACE_GET_OBJECT_EVENT_CODE(RECEIVE, TRCFAILED, QUEUE, pxQueue),     \
        QUEUE, pxQueue);                                                     \
  }
...
#define traceBLOCKING_ON_QUEUE_RECEIVE(pxQueue)                                \
  if (isQueueReceiveHookActuallyPeek) {                                        \
    trcKERNEL_HOOKS_KERNEL_SERVICE(                                            \
        TRACE_GET_OBJECT_EVENT_CODE(PEEK, TRCBLOCK, QUEUE, pxQueue), QUEUE,    \
        pxQueue);                                                              \
  } else {                                                                     \
    trcKERNEL_HOOKS_KERNEL_SERVICE(                                            \
        TRACE_GET_OBJECT_EVENT_CODE(RECEIVE, TRCBLOCK, QUEUE, pxQueue), QUEUE, \
        pxQueue);                                                              \
  }                                                                            \
...
#define traceQUEUE_SEND_FROM_ISR(pxQueue)                                  \
  trcKERNEL_HOOKS_KERNEL_SERVICE_FROM_ISR(                                 \
      xCopyPosition == queueSEND_TO_BACK                                   \
          ? (TRACE_GET_OBJECT_EVENT_CODE(SEND_FROM_ISR, TRCSUCCESS, QUEUE, \
                                         pxQueue))                         \
          : TRACE_QUEUE_SEND_TO_FRONT_FROM_ISR_TRCSUCCESS,                 \
      QUEUE, pxQueue);                                                     \
  trcKERNEL_HOOKS_SET_OBJECT_STATE(QUEUE, pxQueue,                         \
                                   (uint8_t)(pxQueue->uxMessagesWaiting + 1));

/* Called when a message send from interrupt context fails (since the queue was
 * full) */
#undef traceQUEUE_SEND_FROM_ISR_FAILED
#define traceQUEUE_SEND_FROM_ISR_FAILED(pxQueue)                          \
  trcKERNEL_HOOKS_KERNEL_SERVICE_FROM_ISR(                                \
      xCopyPosition == queueSEND_TO_BACK                                  \
          ? (TRACE_GET_OBJECT_EVENT_CODE(SEND_FROM_ISR, TRCFAILED, QUEUE, \
                                         pxQueue))                        \
          : TRACE_QUEUE_SEND_TO_FRONT_FROM_ISR_TRCFAILED,                 \
      QUEUE, pxQueue);
...
#define traceQUEUE_RECEIVE_FROM_ISR(pxQueue)                           \
  trcKERNEL_HOOKS_KERNEL_SERVICE_FROM_ISR(                             \
      TRACE_GET_OBJECT_EVENT_CODE(RECEIVE_FROM_ISR, TRCSUCCESS, QUEUE, \
                                  pxQueue),                            \
      QUEUE, pxQueue);                                                 \
  trcKERNEL_HOOKS_SET_OBJECT_STATE(QUEUE, pxQueue,                     \
                                   (uint8_t)(pxQueue->uxMessagesWaiting - 1));
...
#define traceQUEUE_RECEIVE_FROM_ISR_FAILED(pxQueue)                   \
  trcKERNEL_HOOKS_KERNEL_SERVICE_FROM_ISR(                            \
      TRACE_GET_OBJECT_EVENT_CODE(RECEIVE_FROM_ISR, TRCFAILED, QUEUE, \
                                  pxQueue),                           \
      QUEUE, pxQueue);

#undef traceQUEUE_REGISTRY_ADD
#define traceQUEUE_REGISTRY_ADD(object, name)                        \
  prvTraceSetObjectName(TRACE_GET_OBJECT_TRACE_CLASS(QUEUE, object), \
                        TRACE_GET_OBJECT_NUMBER(QUEUE, object), name);

/* Called in vTaskPrioritySet */
#undef traceTASK_PRIORITY_SET
#define traceTASK_PRIORITY_SET(pxTask, uxNewPriority)             \
  trcKERNEL_HOOKS_TASK_PRIORITY_CHANGE(TASK_PRIORITY_SET, pxTask, \
                                       uxNewPriority);
...
#define traceTASK_PRIORITY_INHERIT(pxTask, uxNewPriority)             \
  trcKERNEL_HOOKS_TASK_PRIORITY_CHANGE(TASK_PRIORITY_INHERIT, pxTask, \
                                       uxNewPriority);
...
#define traceTASK_PRIORITY_DISINHERIT(pxTask, uxNewPriority)             \
  trcKERNEL_HOOKS_TASK_PRIORITY_CHANGE(TASK_PRIORITY_DISINHERIT, pxTask, \
                                       uxNewPriority);
...
extern void vTraceStoreMemMangEvent(uint32_t ecode, uint32_t address,
                                    int32_t size);

/* MALLOC and FREE are always stored, no matter if they happen inside filtered
 * task */
#undef traceMALLOC
#define traceMALLOC(pvAddress, uiSize)                                      \
  if (pvAddress != 0) {                                                     \
    vTraceStoreMemMangEvent(MEM_MALLOC_SIZE, (uint32_t)pvAddress,           \
                            (int32_t)uiSize);                               \
  } else {                                                                  \
    vTraceStoreMemMangEvent(MEM_MALLOC_SIZE_TRCFAILED, (uint32_t)pvAddress, \
                            (int32_t)uiSize);                               \
  }

#undef traceFREE
#define traceFREE(pvAddress, uiSize)                          \
  vTraceStoreMemMangEvent(MEM_FREE_SIZE, (uint32_t)pvAddress, \
                          -((int32_t)uiSize));
...
/* Note that xCommandID can never be tmrCOMMAND_EXECUTE_CALLBACK (-1) since the
 * trace macro is not called in that case */
...
#define traceTIMER_COMMAND_SEND(tmr, xCommandID, xOptionalValue, xReturn)      \
  if (xCommandID > tmrCOMMAND_START_DONT_TRACE) {                              \
    if (xCommandID == tmrCOMMAND_CHANGE_PERIOD) {                              \
      if (xReturn == pdPASS) {                                                 \
        trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(TIMER_CHANGE_PERIOD, TIMER,  \
                                                  tmr, xOptionalValue);        \
      } else {                                                                 \
        trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(                             \
            TIMER_CHANGE_PERIOD_TRCFAILED, TIMER, tmr, xOptionalValue);        \
      }                                                                        \
    } else if ((xCommandID == tmrCOMMAND_DELETE) && (xReturn == pdPASS)) {     \
      trcKERNEL_HOOKS_OBJECT_DELETE(                                           \
          TIMER_DELETE_OBJ,                                                    \
          EVENTGROUP_OBJCLOSE_NAME_TRCSUCCESS +                                \
              TRACE_GET_OBJECT_TRACE_CLASS(TIMER, tmr),                        \
          EVENTGROUP_OBJCLOSE_PROP_TRCSUCCESS +                                \
              TRACE_GET_OBJECT_TRACE_CLASS(TIMER, tmr),                        \
          TIMER, tmr);                                                         \
    } else {                                                                   \
      trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(                               \
          EVENTGROUP_TIMER + (uint32_t)xCommandID +                            \
              ((xReturn == pdPASS) ? 0                                         \
                                   : (TIMER_CREATE_TRCFAILED - TIMER_CREATE)), \
          TIMER, tmr, xOptionalValue);                                         \
    }                                                                          \
  }
...
#define tracePEND_FUNC_CALL(func, arg1, arg2, ret)                    \
  if (ret == pdPASS) {                                                \
    trcKERNEL_HOOKS_KERNEL_SERVICE(PEND_FUNC_CALL, TASK,              \
                                   xTimerGetTimerDaemonTaskHandle()); \
  } else {                                                            \
    trcKERNEL_HOOKS_KERNEL_SERVICE(PEND_FUNC_CALL_TRCFAILED, TASK,    \
                                   xTimerGetTimerDaemonTaskHandle()); \
  }
...
#define tracePEND_FUNC_CALL_FROM_ISR(func, arg1, arg2, ret)       \
  if (!uiInEventGroupSetBitsFromISR)                              \
    prvTraceStoreKernelCall(                                      \
        PEND_FUNC_CALL_FROM_ISR, TRACE_CLASS_TASK,                \
        TRACE_GET_TASK_NUMBER(xTimerGetTimerDaemonTaskHandle())); \
...
#define traceEVENT_GROUP_CREATE_FAILED()                  \
  trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_NUMERIC_PARAM_ONLY( \
      EVENT_GROUP_CREATE_TRCFAILED, 0);
...
#define traceEVENT_GROUP_SYNC_BLOCK(eg, bitsToSet, bitsToWaitFor)      \
  trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(EVENT_GROUP_SYNC_TRCBLOCK, \
                                            EVENTGROUP, eg, bitsToWaitFor);

#undef traceEVENT_GROUP_SYNC_END
#define traceEVENT_GROUP_SYNC_END(eg, bitsToSet, bitsToWaitFor, wasTimeout)   \
  if (wasTimeout) {                                                           \
    trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(EVENT_GROUP_SYNC_END_TRCFAILED, \
                                              EVENTGROUP, eg, bitsToWaitFor); \
  } else {                                                                    \
    trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(EVENT_GROUP_SYNC_END,           \
                                              EVENTGROUP, eg, bitsToWaitFor); \
  }

#undef traceEVENT_GROUP_WAIT_BITS_BLOCK
#define traceEVENT_GROUP_WAIT_BITS_BLOCK(eg, bitsToWaitFor)                 \
  trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(EVENT_GROUP_WAIT_BITS_TRCBLOCK, \
                                            EVENTGROUP, eg, bitsToWaitFor); \
  trcKERNEL_HOOKS_SET_TASK_INSTANCE_FINISHED();
...
#define traceEVENT_GROUP_WAIT_BITS_END(eg, bitsToWaitFor, wasTimeout)         \
  if (wasTimeout) {                                                           \
    trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(                                \
        EVENT_GROUP_WAIT_BITS_END_TRCFAILED, EVENTGROUP, eg, bitsToWaitFor);  \
  } else {                                                                    \
    trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(EVENT_GROUP_WAIT_BITS_END,      \
                                              EVENTGROUP, eg, bitsToWaitFor); \
  }

#undef traceEVENT_GROUP_CLEAR_BITS
#define traceEVENT_GROUP_CLEAR_BITS(eg, bitsToClear)                \
  trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(EVENT_GROUP_CLEAR_BITS, \
                                            EVENTGROUP, eg, bitsToClear);

#undef traceEVENT_GROUP_CLEAR_BITS_FROM_ISR
#define traceEVENT_GROUP_CLEAR_BITS_FROM_ISR(eg, bitsToClear) \
  trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM_FROM_ISR(         \
      EVENT_GROUP_CLEAR_BITS_FROM_ISR, EVENTGROUP, eg, bitsToClear);

#undef traceEVENT_GROUP_SET_BITS
#define traceEVENT_GROUP_SET_BITS(eg, bitsToSet)                              \
  trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(EVENT_GROUP_SET_BITS, EVENTGROUP, \
                                            eg, bitsToSet);

#undef traceEVENT_GROUP_SET_BITS_FROM_ISR
#define traceEVENT_GROUP_SET_BITS_FROM_ISR(eg, bitsToSet)        \
  trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM_FROM_ISR(            \
      EVENT_GROUP_SET_BITS_FROM_ISR, EVENTGROUP, eg, bitsToSet); \
...
#define traceTASK_NOTIFY_TAKE()                                              \
  if (pxCurrentTCB->eNotifyState == eNotified) {                             \
    trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(TRACE_TASK_NOTIFY_TAKE, TASK,  \
                                              pxCurrentTCB, xTicksToWait);   \
  } else {                                                                   \
    trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(                               \
        TRACE_TASK_NOTIFY_TAKE_TRCFAILED, TASK, pxCurrentTCB, xTicksToWait); \
  }
...
#define traceTASK_NOTIFY_TAKE_BLOCK()                                          \
  trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(TRACE_TASK_NOTIFY_TAKE_TRCBLOCK,   \
                                            TASK, pxCurrentTCB, xTicksToWait); \
  trcKERNEL_HOOKS_SET_TASK_INSTANCE_FINISHED();
  ...
  if (TRACE_GET_OBJECT_FILTER(TASK, pxCurrentTCB) & CurrentFilterMask) { \
    if (pxCurrentTCB->eNotifyState == eNotified)                         \
      prvTraceStoreKernelCallWithParam(                                  \
          TRACE_TASK_NOTIFY_WAIT, TRACE_CLASS_TASK,                      \
          TRACE_GET_TASK_NUMBER(pxCurrentTCB), xTicksToWait);            \
    else                                                                 \
      prvTraceStoreKernelCallWithParam(                                  \
          TRACE_TASK_NOTIFY_WAIT_TRCFAILED, TRACE_CLASS_TASK,            \
          TRACE_GET_TASK_NUMBER(pxCurrentTCB), xTicksToWait);            \
  }
#elif (TRC_CFG_FREERTOS_VERSION < TRC_FREERTOS_VERSION_10_4_0)
#define traceTASK_NOTIFY_WAIT()                                          \
...
#define traceTASK_NOTIFY_WAIT_BLOCK()                                  \
  if (TRACE_GET_OBJECT_FILTER(TASK, pxCurrentTCB) & CurrentFilterMask) \
    prvTraceStoreKernelCallWithParam(                                  \
        TRACE_TASK_NOTIFY_WAIT_TRCBLOCK, TRACE_CLASS_TASK,             \
        TRACE_GET_TASK_NUMBER(pxCurrentTCB), xTicksToWait);            \
  trcKERNEL_HOOKS_SET_TASK_INSTANCE_FINISHED();
#else /* TRC_CFG_FREERTOS_VERSION < TRC_FREERTOS_VERSION_10_4_0 */
...
#define traceTASK_NOTIFY()                                                \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &           \
      CurrentFilterMask)                                                  \
    if (TRACE_GET_OBJECT_FILTER(TASK, xTaskToNotify) & CurrentFilterMask) \
      prvTraceStoreKernelCall(TRACE_TASK_NOTIFY, TRACE_CLASS_TASK,        \
                              TRACE_GET_TASK_NUMBER(xTaskToNotify));
#else /* TRC_CFG_FREERTOS_VERSION < TRC_FREERTOS_VERSION_10_4_0 */
...
#define traceTASK_NOTIFY_FROM_ISR()                                       \
  if (TRACE_GET_OBJECT_FILTER(TASK, xTaskToNotify) & CurrentFilterMask)   \
    prvTraceStoreKernelCall(TRACE_TASK_NOTIFY_FROM_ISR, TRACE_CLASS_TASK, \
                            TRACE_GET_TASK_NUMBER(xTaskToNotify));
#else /* TRC_CFG_FREERTOS_VERSION < TRC_FREERTOS_VERSION_10_4_0 */
...
#define traceTASK_NOTIFY_GIVE_FROM_ISR()                                       \
  if (TRACE_GET_OBJECT_FILTER(TASK, xTaskToNotify) & CurrentFilterMask)        \
    prvTraceStoreKernelCall(TRACE_TASK_NOTIFY_GIVE_FROM_ISR, TRACE_CLASS_TASK, \
                            TRACE_GET_TASK_NUMBER(xTaskToNotify));
#else /* TRC_CFG_FREERTOS_VERSION < TRC_FREERTOS_VERSION_10_4_0 */
...
#define traceSTREAM_BUFFER_CREATE(pxStreamBuffer, xIsMessageBuffer)     \
  trcKERNEL_HOOKS_OBJECT_CREATE(                                        \
      TRACE_GET_OBJECT_EVENT_CODE(CREATE_OBJ, TRCSUCCESS, STREAMBUFFER, \
                                  pxStreamBuffer),                      \
      STREAMBUFFER, pxStreamBuffer);

#undef traceSTREAM_BUFFER_CREATE_FAILED
#define traceSTREAM_BUFFER_CREATE_FAILED(xIsMessageBuffer)            \
  trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_NUMERIC_PARAM_ONLY(             \
      TRACE_GET_CLASS_EVENT_CODE(CREATE_OBJ, TRCFAILED, STREAMBUFFER, \
                                 xIsMessageBuffer),                   \
      0);
...
#define traceSTREAM_BUFFER_DELETE(xStreamBuffer)                           \
  trcKERNEL_HOOKS_OBJECT_DELETE(                                           \
      TRACE_GET_OBJECT_EVENT_CODE(DELETE_OBJ, TRCSUCCESS, STREAMBUFFER,    \
                                  xStreamBuffer),                          \
      TRACE_GET_OBJECT_EVENT_CODE(OBJCLOSE_NAME, TRCSUCCESS, STREAMBUFFER, \
                                  xStreamBuffer),                          \
      TRACE_GET_OBJECT_EVENT_CODE(OBJCLOSE_PROP, TRCSUCCESS, STREAMBUFFER, \
                                  xStreamBuffer),                          \
      STREAMBUFFER, xStreamBuffer);

#undef traceSTREAM_BUFFER_RESET
#define traceSTREAM_BUFFER_RESET(xStreamBuffer)                            \
  trcKERNEL_HOOKS_KERNEL_SERVICE(prvGetStreamBufferType(xStreamBuffer) > 0 \
                                     ? TRACE_MESSAGEBUFFER_RESET           \
                                     : TRACE_STREAMBUFFER_RESET,           \
                                 STREAMBUFFER, xStreamBuffer);             \
...
#define traceSTREAM_BUFFER_SEND(xStreamBuffer, xReturn)           \
  trcKERNEL_HOOKS_KERNEL_SERVICE(                                 \
      TRACE_GET_OBJECT_EVENT_CODE(SEND, TRCSUCCESS, STREAMBUFFER, \
                                  xStreamBuffer),                 \
      STREAMBUFFER, xStreamBuffer);                               \
  trcKERNEL_HOOKS_SET_OBJECT_STATE(STREAMBUFFER, xStreamBuffer,   \
                                   prvBytesInBuffer(xStreamBuffer));

#undef traceBLOCKING_ON_STREAM_BUFFER_SEND
#define traceBLOCKING_ON_STREAM_BUFFER_SEND(xStreamBuffer)      \
  trcKERNEL_HOOKS_KERNEL_SERVICE(                               \
      TRACE_GET_OBJECT_EVENT_CODE(SEND, TRCBLOCK, STREAMBUFFER, \
                                  xStreamBuffer),               \
      STREAMBUFFER, xStreamBuffer);
...
#define traceSTREAM_BUFFER_SEND_FAILED(xStreamBuffer)            \
  trcKERNEL_HOOKS_KERNEL_SERVICE(                                \
      TRACE_GET_OBJECT_EVENT_CODE(SEND, TRCFAILED, STREAMBUFFER, \
                                  xStreamBuffer),                \
      STREAMBUFFER, xStreamBuffer);
...
#define traceSTREAM_BUFFER_RECEIVE(xStreamBuffer, xReceivedLength)   \
  trcKERNEL_HOOKS_KERNEL_SERVICE(                                    \
      TRACE_GET_OBJECT_EVENT_CODE(RECEIVE, TRCSUCCESS, STREAMBUFFER, \
                                  xStreamBuffer),                    \
      STREAMBUFFER, xStreamBuffer);                                  \
  trcKERNEL_HOOKS_SET_OBJECT_STATE(STREAMBUFFER, xStreamBuffer,      \
                                   prvBytesInBuffer(xStreamBuffer));
...
#define traceSTREAM_BUFFER_RECEIVE_FAILED(xStreamBuffer)            \
  trcKERNEL_HOOKS_KERNEL_SERVICE(                                   \
      TRACE_GET_OBJECT_EVENT_CODE(RECEIVE, TRCFAILED, STREAMBUFFER, \
                                  xStreamBuffer),                   \
      STREAMBUFFER, xStreamBuffer);
...
#define traceSTREAM_BUFFER_SEND_FROM_ISR(xStreamBuffer, xReturn)             \
  if (xReturn > (size_t)0) {                                                 \
    trcKERNEL_HOOKS_KERNEL_SERVICE_FROM_ISR(                                 \
        TRACE_GET_OBJECT_EVENT_CODE(SEND_FROM_ISR, TRCSUCCESS, STREAMBUFFER, \
                                    xStreamBuffer),                          \
        STREAMBUFFER, xStreamBuffer);                                        \
    trcKERNEL_HOOKS_SET_OBJECT_STATE(STREAMBUFFER, xStreamBuffer,            \
                                     prvBytesInBuffer(xStreamBuffer));       \
    ...
    trcKERNEL_HOOKS_KERNEL_SERVICE_FROM_ISR(                                 \
        TRACE_GET_OBJECT_EVENT_CODE(SEND_FROM_ISR, TRCFAILED, STREAMBUFFER,  \
                                    xStreamBuffer),                          \
        STREAMBUFFER, xStreamBuffer);                                        \
  }
...
#define traceSTREAM_BUFFER_RECEIVE_FROM_ISR(xStreamBuffer, xReceivedLength)    \
  if (xReceivedLength > (size_t)0) {                                           \
    trcKERNEL_HOOKS_KERNEL_SERVICE_FROM_ISR(                                   \
        TRACE_GET_OBJECT_EVENT_CODE(RECEIVE_FROM_ISR, TRCSUCCESS,              \
                                    STREAMBUFFER, xStreamBuffer),              \
        STREAMBUFFER, xStreamBuffer);                                          \
    trcKERNEL_HOOKS_SET_OBJECT_STATE(STREAMBUFFER, xStreamBuffer,              \
                                     prvBytesInBuffer(xStreamBuffer));         \
    ...
    trcKERNEL_HOOKS_KERNEL_SERVICE_FROM_ISR(                                   \
        TRACE_GET_OBJECT_EVENT_CODE(RECEIVE_FROM_ISR, TRCFAILED, STREAMBUFFER, \
                                    xStreamBuffer),                            \
        STREAMBUFFER, xStreamBuffer);                                          \
  }
...
/* A macro that will update the tick count when returning from tickless idle */
#undef traceINCREASE_TICK_COUNT
/* Note: This can handle time adjustments of max 2^32 ticks, i.e., 35 seconds at
 * 120 MHz. Thus, tick-less idle periods longer than 2^32 ticks will appear
 * "compressed" on the time line.*/
#define traceINCREASE_TICK_COUNT(xCount) \
  {                                      \
    extern uint32_t uiTraceTickCount;    \
    uiTraceTickCount += xCount;          \
  }

#if (TRC_CFG_INCLUDE_OSTICK_EVENTS == 1)
#define OS_TICK_EVENT(uxSchedulerSuspended, xTickCount)                  \
  if (uxSchedulerSuspended == (unsigned portBASE_TYPE)pdFALSE) {         \
    prvTraceStoreEvent1(PSF_EVENT_NEW_TIME, (uint32_t)(xTickCount + 1)); \
  }
#else
#define OS_TICK_EVENT(uxSchedulerSuspended, xTickCount)
#endif
...
#define traceTASK_SWITCHED_IN()                                            \
  ...
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &            \
    ...
    if (prvIsNewTCB(pxCurrentTCB)) {                                       \
      prvTraceStoreEvent2(PSF_EVENT_TASK_ACTIVATE, (uint32_t)pxCurrentTCB, \
...
#define traceTASK_CREATE(pxNewTCB)                                        \
  if (pxNewTCB != NULL) {                                                 \
    ...
    prvTraceStoreStringEvent(1, PSF_EVENT_OBJ_NAME, pxNewTCB->pcTaskName, \
                             pxNewTCB);                                   \
    TRACE_SET_OBJECT_FILTER(TASK, pxNewTCB, CurrentFilterGroup);          \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &         \
        CurrentFilterMask)                                                \
      if (TRACE_GET_OBJECT_FILTER(TASK, pxNewTCB) & CurrentFilterMask)    \
        prvTraceStoreEvent2(PSF_EVENT_TASK_CREATE, (uint32_t)pxNewTCB,    \
                            pxNewTCB->uxPriority);                        \
  }
#else /* TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_9_0_0 */
#define traceTASK_CREATE(pxNewTCB)                                       \
  if (pxNewTCB != NULL) {                                                \
    ...
    prvTraceStoreStringEvent(1, PSF_EVENT_OBJ_NAME, (const char*)pcName, \
                             pxNewTCB);                                  \
    TRACE_SET_OBJECT_FILTER(TASK, pxNewTCB, CurrentFilterGroup);         \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &        \
        CurrentFilterMask)                                               \
...
/* Called on vTaskDelete */
#undef traceTASK_DELETE  // We don't allow for filtering out "delete" events.
                         // They are important and not very frequent. Moreover,
                         // we can't exclude create events, so this should be
                         // symmetrical.
#define traceTASK_DELETE(pxTaskToDelete)                                   \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &            \
      CurrentFilterMask)                                                   \
    if (TRACE_GET_OBJECT_FILTER(TASK, pxTaskToDelete) & CurrentFilterMask) \
      prvTraceStoreEvent2(                                                 \
          PSF_EVENT_TASK_DELETE, (uint32_t)pxTaskToDelete,                 \
          (pxTaskToDelete != NULL) ? (pxTaskToDelete->uxPriority) : 0);    \
...
#define traceQUEUE_CREATE_HELPER()                                     \
  ...
  case queueQUEUE_TYPE_RECURSIVE_MUTEX:                                \
    prvTraceStoreEvent1(PSF_EVENT_MUTEX_RECURSIVE_CREATE,              \
                        (uint32_t)pxNewQueue);                         \
    break;
#else
#define traceQUEUE_CREATE_HELPER()
    ...
    if (TRACE_GET_OBJECT_FILTER(QUEUE, pxNewQueue) & CurrentFilterMask) {   \
      switch (pxNewQueue->ucQueueType) {                                    \
        case queueQUEUE_TYPE_BASE:                                          \
          prvTraceStoreEvent2(PSF_EVENT_QUEUE_CREATE, (uint32_t)pxNewQueue, \
                              uxQueueLength);                               \
          break;                                                            \
        case queueQUEUE_TYPE_BINARY_SEMAPHORE:                              \
          prvTraceStoreEvent1(PSF_EVENT_SEMAPHORE_BINARY_CREATE,            \
                              (uint32_t)pxNewQueue);                        \
          break;                                                            \
...
#undef traceQUEUE_DELETE  // We don't allow for filtering out "delete" events.
                          // They are important and not very frequent. Moreover,
                          // we can't exclude create events, so this should be
                          // symmetrical.
#define traceQUEUE_DELETE(pxQueue)                                     \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &        \
    ...
    if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask) { \
      switch (pxQueue->ucQueueType) {                                  \
        case queueQUEUE_TYPE_BASE:                                     \
          prvTraceStoreEvent2(                                         \
              PSF_EVENT_QUEUE_DELETE, (uint32_t)pxQueue,               \
              (pxQueue != NULL) ? (pxQueue->uxMessagesWaiting) : 0);   \
          break;                                                       \
        case queueQUEUE_TYPE_MUTEX:                                    \
        case queueQUEUE_TYPE_RECURSIVE_MUTEX:                          \
          prvTraceStoreEvent2(                                         \
              PSF_EVENT_MUTEX_DELETE, (uint32_t)pxQueue,               \
              (pxQueue != NULL) ? (pxQueue->uxMessagesWaiting) : 0);   \
          break;                                                       \
        case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                       \
        case queueQUEUE_TYPE_BINARY_SEMAPHORE:                         \
          prvTraceStoreEvent2(                                         \
              PSF_EVENT_SEMAPHORE_DELETE, (uint32_t)pxQueue,           \
              (pxQueue != NULL) ? (pxQueue->uxMessagesWaiting) : 0);   \
          break;                                                       \
      }                                                                \
    }                                                                  \
...
#define traceCREATE_COUNTING_SEMAPHORE()                                      \
  ...
  prvTraceStoreEvent2(PSF_EVENT_SEMAPHORE_COUNTING_CREATE, (uint32_t)xHandle, \
                      uxMaxCount)
#elif (TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_7_5_X)
#define traceCREATE_COUNTING_SEMAPHORE()                             \
  TRACE_SET_OBJECT_FILTER(QUEUE, xHandle, CurrentFilterGroup);       \
...
#define traceCREATE_COUNTING_SEMAPHORE()                              \
  ...
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &       \
    ...
    if (TRACE_GET_OBJECT_FILTER(QUEUE, pxHandle) & CurrentFilterMask) \
      prvTraceStoreEvent2(PSF_EVENT_SEMAPHORE_COUNTING_CREATE,        \
                          (uint32_t)pxHandle, uxCountValue);
#endif /* TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_8_X_X */
...
#define traceCREATE_COUNTING_SEMAPHORE_FAILED()                        \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &        \
      CurrentFilterMask)                                               \
    prvTraceStoreEvent2(PSF_EVENT_SEMAPHORE_COUNTING_CREATE_FAILED, 0, \
                        uxMaxCount);
#elif (TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_7_5_X)
#define traceCREATE_COUNTING_SEMAPHORE_FAILED()                        \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &        \
  ...
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &                \
    ...
    if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask)           \
      switch (pxQueue->ucQueueType) {                                          \
        case queueQUEUE_TYPE_BASE:                                             \
          prvTraceStoreEvent2(                                                 \
              xCopyPosition == queueSEND_TO_BACK ? PSF_EVENT_QUEUE_SEND        \
                                                 : PSF_EVENT_QUEUE_SEND_FRONT, \
              (uint32_t)pxQueue, pxQueue->uxMessagesWaiting + 1);              \
          break;                                                               \
        case queueQUEUE_TYPE_BINARY_SEMAPHORE:                                 \
        case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                               \
          prvTraceStoreEvent2(PSF_EVENT_SEMAPHORE_GIVE, (uint32_t)pxQueue,     \
...
#define traceQUEUE_SET_SEND(pxQueue)                                 \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &      \
      CurrentFilterMask)                                             \
    if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask) \
      prvTraceStoreEvent2(PSF_EVENT_QUEUE_SEND, (uint32_t)pxQueue,   \
                          pxQueue->uxMessagesWaiting + 1);
...
#define traceQUEUE_SEND_FAILED(pxQueue)                                        \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &                \
    ...
    if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask)           \
      switch (pxQueue->ucQueueType) {                                          \
        case queueQUEUE_TYPE_BASE:                                             \
          prvTraceStoreEvent2(xCopyPosition == queueSEND_TO_BACK               \
                                  ? PSF_EVENT_QUEUE_SEND_FAILED                \
                                  : PSF_EVENT_QUEUE_SEND_FRONT_FAILED,         \
                              (uint32_t)pxQueue, pxQueue->uxMessagesWaiting);  \
          break;                                                               \
        case queueQUEUE_TYPE_BINARY_SEMAPHORE:                                 \
        case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                               \
          prvTraceStoreEvent2(PSF_EVENT_SEMAPHORE_GIVE_FAILED,                 \
                              (uint32_t)pxQueue, pxQueue->uxMessagesWaiting);  \
          break;                                                               \
        case queueQUEUE_TYPE_MUTEX:                                            \
        case queueQUEUE_TYPE_RECURSIVE_MUTEX:                                  \
  ...
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &               \
    ...
    if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask)          \
      switch (pxQueue->ucQueueType) {                                         \
        case queueQUEUE_TYPE_BASE:                                            \
          prvTraceStoreEvent2(xCopyPosition == queueSEND_TO_BACK              \
                                  ? PSF_EVENT_QUEUE_SEND_BLOCK                \
                                  : PSF_EVENT_QUEUE_SEND_FRONT_BLOCK,         \
                              (uint32_t)pxQueue, pxQueue->uxMessagesWaiting); \
          break;                                                              \
        case queueQUEUE_TYPE_BINARY_SEMAPHORE:                                \
        case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                              \
          prvTraceStoreEvent2(PSF_EVENT_SEMAPHORE_GIVE_BLOCK,                 \
                              (uint32_t)pxQueue, pxQueue->uxMessagesWaiting); \
          break;                                                              \
        case queueQUEUE_TYPE_MUTEX:                                           \
        case queueQUEUE_TYPE_RECURSIVE_MUTEX:                                 \
...
#define traceQUEUE_SEND_FROM_ISR(pxQueue)                             \
  if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask)    \
    switch (pxQueue->ucQueueType) {                                   \
      case queueQUEUE_TYPE_BASE:                                      \
        prvTraceStoreEvent2(xCopyPosition == queueSEND_TO_BACK        \
                                ? PSF_EVENT_QUEUE_SEND_FROMISR        \
                                : PSF_EVENT_QUEUE_SEND_FRONT_FROMISR, \
                            (uint32_t)pxQueue,                        \
                            pxQueue->uxMessagesWaiting + 1);          \
        break;                                                        \
      case queueQUEUE_TYPE_BINARY_SEMAPHORE:                          \
      case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                        \
        prvTraceStoreEvent2(PSF_EVENT_SEMAPHORE_GIVE_FROMISR,         \
                            (uint32_t)pxQueue,                        \
                            pxQueue->uxMessagesWaiting + 1);          \
        break;                                                        \
    }
...
#define traceQUEUE_SEND_FROM_ISR_FAILED(pxQueue)                             \
  if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask)           \
    switch (pxQueue->ucQueueType) {                                          \
      case queueQUEUE_TYPE_BASE:                                             \
        prvTraceStoreEvent2(xCopyPosition == queueSEND_TO_BACK               \
                                ? PSF_EVENT_QUEUE_SEND_FROMISR_FAILED        \
                                : PSF_EVENT_QUEUE_SEND_FRONT_FROMISR_FAILED, \
                            (uint32_t)pxQueue, pxQueue->uxMessagesWaiting);  \
        break;                                                               \
      case queueQUEUE_TYPE_BINARY_SEMAPHORE:                                 \
      case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                               \
        prvTraceStoreEvent2(PSF_EVENT_SEMAPHORE_GIVE_FROMISR_FAILED,         \
                            (uint32_t)pxQueue, pxQueue->uxMessagesWaiting);  \
        break;                                                               \
    }
  ...
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &                \
    ...
    if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask)           \
      switch (pxQueue->ucQueueType) {                                          \
        case queueQUEUE_TYPE_BASE:                                             \
          if (isQueueReceiveHookActuallyPeek)                                  \
            prvTraceStoreEvent3(PSF_EVENT_QUEUE_PEEK, (uint32_t)pxQueue,       \
                                xTicksToWait, pxQueue->uxMessagesWaiting - 1); \
          else                                                                 \
            prvTraceStoreEvent3(PSF_EVENT_QUEUE_RECEIVE, (uint32_t)pxQueue,    \
                                xTicksToWait, pxQueue->uxMessagesWaiting - 1); \
          break;                                                               \
        case queueQUEUE_TYPE_BINARY_SEMAPHORE:                                 \
        case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                               \
          if (isQueueReceiveHookActuallyPeek)                                  \
            prvTraceStoreEvent3(PSF_EVENT_SEMAPHORE_PEEK, (uint32_t)pxQueue,   \
                                xTicksToWait, pxQueue->uxMessagesWaiting - 1); \
          else                                                                 \
            prvTraceStoreEvent3(PSF_EVENT_SEMAPHORE_TAKE, (uint32_t)pxQueue,   \
                                xTicksToWait, pxQueue->uxMessagesWaiting - 1); \
          break;                                                               \
        case queueQUEUE_TYPE_MUTEX:                                            \
        case queueQUEUE_TYPE_RECURSIVE_MUTEX:                                  \
          if (isQueueReceiveHookActuallyPeek)                                  \
            prvTraceStoreEvent2(PSF_EVENT_MUTEX_PEEK, (uint32_t)pxQueue,       \
                                xTicksToWait);                                 \
          else                                                                 \
            prvTraceStoreEvent2(PSF_EVENT_MUTEX_TAKE, (uint32_t)pxQueue,       \
                                xTicksToWait);                                 \
          break;                                                               \
      }
  ...
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &                \
    ...
    if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask)           \
      switch (pxQueue->ucQueueType) {                                          \
        case queueQUEUE_TYPE_BASE:                                             \
          prvTraceStoreEvent3(                                                 \
              isQueueReceiveHookActuallyPeek ? PSF_EVENT_QUEUE_PEEK_FAILED     \
                                             : PSF_EVENT_QUEUE_RECEIVE_FAILED, \
              (uint32_t)pxQueue, xTicksToWait, pxQueue->uxMessagesWaiting);    \
          break;                                                               \
        case queueQUEUE_TYPE_BINARY_SEMAPHORE:                                 \
        case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                               \
          prvTraceStoreEvent3(isQueueReceiveHookActuallyPeek                   \
                                  ? PSF_EVENT_SEMAPHORE_PEEK_FAILED            \
                                  : PSF_EVENT_SEMAPHORE_TAKE_FAILED,           \
                              (uint32_t)pxQueue, xTicksToWait,                 \
                              pxQueue->uxMessagesWaiting);                     \
          break;                                                               \
        case queueQUEUE_TYPE_MUTEX:                                            \
        case queueQUEUE_TYPE_RECURSIVE_MUTEX:                                  \
          prvTraceStoreEvent2(isQueueReceiveHookActuallyPeek                   \
                                  ? PSF_EVENT_MUTEX_PEEK_FAILED                \
                                  : PSF_EVENT_MUTEX_TAKE_FAILED,               \
                              (uint32_t)pxQueue, xTicksToWait);                \
          break;                                                               \
      }
  ...
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &                \
    ...
    if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask)           \
      switch (pxQueue->ucQueueType) {                                          \
        case queueQUEUE_TYPE_BASE:                                             \
          prvTraceStoreEvent3(                                                 \
              isQueueReceiveHookActuallyPeek ? PSF_EVENT_QUEUE_PEEK_BLOCK      \
                                             : PSF_EVENT_QUEUE_RECEIVE_BLOCK,  \
              (uint32_t)pxQueue, xTicksToWait, pxQueue->uxMessagesWaiting);    \
          break;                                                               \
        case queueQUEUE_TYPE_BINARY_SEMAPHORE:                                 \
        case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                               \
          prvTraceStoreEvent3(                                                 \
              isQueueReceiveHookActuallyPeek ? PSF_EVENT_SEMAPHORE_PEEK_BLOCK  \
                                             : PSF_EVENT_SEMAPHORE_TAKE_BLOCK, \
              (uint32_t)pxQueue, xTicksToWait, pxQueue->uxMessagesWaiting);    \
          break;                                                               \
        case queueQUEUE_TYPE_MUTEX:                                            \
        case queueQUEUE_TYPE_RECURSIVE_MUTEX:                                  \
          prvTraceStoreEvent2(isQueueReceiveHookActuallyPeek                   \
                                  ? PSF_EVENT_MUTEX_PEEK_BLOCK                 \
                                  : PSF_EVENT_MUTEX_TAKE_BLOCK,                \
                              (uint32_t)pxQueue, xTicksToWait);                \
          break;                                                               \
      }
...
#define traceQUEUE_PEEK_FAILED(pxQueue)                                       \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &               \
    ...
    if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask)          \
      switch (pxQueue->ucQueueType) {                                         \
        case queueQUEUE_TYPE_BASE:                                            \
          prvTraceStoreEvent3(PSF_EVENT_QUEUE_PEEK_FAILED, (uint32_t)pxQueue, \
                              xTicksToWait, pxQueue->uxMessagesWaiting);      \
          break;                                                              \
        case queueQUEUE_TYPE_BINARY_SEMAPHORE:                                \
        case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                              \
          prvTraceStoreEvent3(PSF_EVENT_SEMAPHORE_PEEK_FAILED,                \
                              (uint32_t)pxQueue, xTicksToWait,                \
                              pxQueue->uxMessagesWaiting);                    \
          break;                                                              \
        case queueQUEUE_TYPE_MUTEX:                                           \
        case queueQUEUE_TYPE_RECURSIVE_MUTEX:                                 \
          prvTraceStoreEvent2(PSF_EVENT_MUTEX_PEEK_FAILED, (uint32_t)pxQueue, \
                              xTicksToWait);                                  \
          break;                                                              \
      }
...
#define traceBLOCKING_ON_QUEUE_PEEK(pxQueue)                                 \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &              \
    ...
    if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask)         \
      switch (pxQueue->ucQueueType) {                                        \
        case queueQUEUE_TYPE_BASE:                                           \
          prvTraceStoreEvent3(PSF_EVENT_QUEUE_PEEK_BLOCK, (uint32_t)pxQueue, \
                              xTicksToWait, pxQueue->uxMessagesWaiting);     \
          break;                                                             \
        case queueQUEUE_TYPE_BINARY_SEMAPHORE:                               \
        case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                             \
          prvTraceStoreEvent3(PSF_EVENT_SEMAPHORE_PEEK_BLOCK,                \
                              (uint32_t)pxQueue, xTicksToWait,               \
                              pxQueue->uxMessagesWaiting);                   \
          break;                                                             \
        case queueQUEUE_TYPE_MUTEX:                                          \
        case queueQUEUE_TYPE_RECURSIVE_MUTEX:                                \
          prvTraceStoreEvent2(PSF_EVENT_MUTEX_PEEK_BLOCK, (uint32_t)pxQueue, \
                              xTicksToWait);                                 \
          break;                                                             \
      }
...
#define traceQUEUE_RECEIVE_FROM_ISR(pxQueue)                       \
  if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask) \
    switch (pxQueue->ucQueueType) {                                \
      case queueQUEUE_TYPE_BASE:                                   \
        prvTraceStoreEvent2(PSF_EVENT_QUEUE_RECEIVE_FROMISR,       \
                            (uint32_t)pxQueue,                     \
                            pxQueue->uxMessagesWaiting - 1);       \
        break;                                                     \
      case queueQUEUE_TYPE_BINARY_SEMAPHORE:                       \
      case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                     \
        prvTraceStoreEvent2(PSF_EVENT_SEMAPHORE_TAKE_FROMISR,      \
                            (uint32_t)pxQueue,                     \
                            pxQueue->uxMessagesWaiting - 1);       \
        break;                                                     \
    }
...
#define traceQUEUE_RECEIVE_FROM_ISR_FAILED(pxQueue)                         \
  if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask)          \
    switch (pxQueue->ucQueueType) {                                         \
      case queueQUEUE_TYPE_BASE:                                            \
        prvTraceStoreEvent2(PSF_EVENT_QUEUE_RECEIVE_FROMISR_FAILED,         \
                            (uint32_t)pxQueue, pxQueue->uxMessagesWaiting); \
        break;                                                              \
      case queueQUEUE_TYPE_BINARY_SEMAPHORE:                                \
      case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                              \
        prvTraceStoreEvent2(PSF_EVENT_SEMAPHORE_TAKE_FROMISR_FAILED,        \
                            (uint32_t)pxQueue, pxQueue->uxMessagesWaiting); \
        break;                                                              \
    }
...
#define traceTASK_PRIORITY_SET(pxTask, uxNewPriority)                \
  ...
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &      \
    ...
    if (TRACE_GET_OBJECT_FILTER(TASK, pxTask) & CurrentFilterMask)   \
      prvTraceStoreEvent2(PSF_EVENT_TASK_PRIORITY, (uint32_t)pxTask, \
                          uxNewPriority);
...
#define traceTASK_PRIORITY_INHERIT(pxTask, uxNewPriority)                \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &          \
      CurrentFilterMask)                                                 \
    if (TRACE_GET_OBJECT_FILTER(TASK, pxTask) & CurrentFilterMask)       \
      prvTraceStoreEvent2(PSF_EVENT_TASK_PRIO_INHERIT, (uint32_t)pxTask, \
                          uxNewPriority);
...
#define traceTASK_PRIORITY_DISINHERIT(pxTask, uxNewPriority)                \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &             \
      CurrentFilterMask)                                                    \
    if (TRACE_GET_OBJECT_FILTER(TASK, pxTask) & CurrentFilterMask)          \
      prvTraceStoreEvent2(PSF_EVENT_TASK_PRIO_DISINHERIT, (uint32_t)pxTask, \
                          uxNewPriority);
...
#define traceTASK_RESUME_FROM_ISR(pxTaskToResume)                        \
  if (TRACE_GET_OBJECT_FILTER(TASK, pxTaskToResume) & CurrentFilterMask) \
    prvTraceStoreEvent1(PSF_EVENT_TASK_RESUME_FROMISR,                   \
                        (uint32_t)pxTaskToResume);
...
#define traceMALLOC(pvAddress, uiSize)                                    \
  ...
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &           \
    ...
    if (pvAddress != 0) {                                                 \
      ...
      prvTraceStoreEvent2(PSF_EVENT_MALLOC_FAILED, (uint32_t)pvAddress,   \
                          uiSize);                                        \
    }                                                                     \
  }
...
#define traceFREE(pvAddress, uiSize)                                          \
  ...
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &               \
      CurrentFilterMask)                                                      \
    prvTraceStoreEvent2(                                                      \
        PSF_EVENT_FREE, (uint32_t)pvAddress,                                  \
        (uint32_t)(0 - uiSize)); /* "0 -" instead of just "-" to get rid of a \
                                    warning... */
...
#define traceTIMER_CREATE(tmr)                                   \
  ...
  prvTraceStoreStringEvent(1, PSF_EVENT_OBJ_NAME,                \
                           (const char*)tmr->pcTimerName, tmr);  \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &  \
      CurrentFilterMask)                                         \
    if (TRACE_GET_OBJECT_FILTER(TIMER, tmr) & CurrentFilterMask) \
      prvTraceStoreEvent2(PSF_EVENT_TIMER_CREATE, (uint32_t)tmr, \
                          tmr->xTimerPeriodInTicks);
...
#define traceTIMER_COMMAND_SEND_8_0_CASES(tmr)                              \
  case tmrCOMMAND_RESET:                                                    \
    prvTraceStoreEvent2((xReturn == pdPASS) ? PSF_EVENT_TIMER_RESET         \
                                            : PSF_EVENT_TIMER_RESET_FAILED, \
                        (uint32_t)tmr, xOptionalValue);                     \
    break;                                                                  \
  case tmrCOMMAND_START_FROM_ISR:                                           \
    prvTraceStoreEvent2((xReturn == pdPASS)                                 \
                            ? PSF_EVENT_TIMER_START_FROMISR                 \
                            : PSF_EVENT_TIMER_START_FROMISR_FAILED,         \
                        (uint32_t)tmr, xOptionalValue);                     \
    break;                                                                  \
  case tmrCOMMAND_RESET_FROM_ISR:                                           \
    prvTraceStoreEvent2((xReturn == pdPASS)                                 \
                            ? PSF_EVENT_TIMER_RESET_FROMISR                 \
                            : PSF_EVENT_TIMER_RESET_FROMISR_FAILED,         \
                        (uint32_t)tmr, xOptionalValue);                     \
    break;                                                                  \
  case tmrCOMMAND_STOP_FROM_ISR:                                            \
    prvTraceStoreEvent2((xReturn == pdPASS)                                 \
                            ? PSF_EVENT_TIMER_STOP_FROMISR                  \
                            : PSF_EVENT_TIMER_STOP_FROMISR_FAILED,          \
                        (uint32_t)tmr, xOptionalValue);                     \
    break;                                                                  \
  case tmrCOMMAND_CHANGE_PERIOD_FROM_ISR:                                   \
    prvTraceStoreEvent2((xReturn == pdPASS)                                 \
                            ? PSF_EVENT_TIMER_CHANGEPERIOD_FROMISR          \
                            : PSF_EVENT_TIMER_CHANGEPERIOD_FROMISR_FAILED,  \
                        (uint32_t)tmr, xOptionalValue);                     \
    break;
#else /* TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_8_X_X */
#define traceTIMER_COMMAND_SEND_8_0_CASES(tmr)
    ...
    if (TRACE_GET_OBJECT_FILTER(TIMER, tmr) & CurrentFilterMask)          \
      switch (xCommandID) {                                               \
        case tmrCOMMAND_START:                                            \
          prvTraceStoreEvent1((xReturn == pdPASS)                         \
                                  ? PSF_EVENT_TIMER_START                 \
                                  : PSF_EVENT_TIMER_START_FAILED,         \
                              (uint32_t)tmr);                             \
          break;                                                          \
        case tmrCOMMAND_STOP:                                             \
          prvTraceStoreEvent1((xReturn == pdPASS)                         \
                                  ? PSF_EVENT_TIMER_STOP                  \
                                  : PSF_EVENT_TIMER_STOP_FAILED,          \
                              (uint32_t)tmr);                             \
          break;                                                          \
        case tmrCOMMAND_CHANGE_PERIOD:                                    \
          prvTraceStoreEvent2((xReturn == pdPASS)                         \
                                  ? PSF_EVENT_TIMER_CHANGEPERIOD          \
                                  : PSF_EVENT_TIMER_CHANGEPERIOD_FAILED,  \
                              (uint32_t)tmr, xOptionalValue);             \
          break;                                                          \
        case tmrCOMMAND_DELETE:                                           \
          prvTraceStoreEvent1((xReturn == pdPASS)                         \
                                  ? PSF_EVENT_TIMER_DELETE                \
                                  : PSF_EVENT_TIMER_DELETE_FAILED,        \
                              (uint32_t)tmr);                             \
          break;                                                          \
...
#define traceTIMER_EXPIRED(tmr)                                  \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &  \
      CurrentFilterMask)                                         \
    if (TRACE_GET_OBJECT_FILTER(TIMER, tmr) & CurrentFilterMask) \
      prvTraceStoreEvent2(PSF_EVENT_TIMER_EXPIRED,               \
                          (uint32_t)tmr->pxCallbackFunction,     \
                          (uint32_t)tmr->pvTimerID);
...
#undef tracePEND_FUNC_CALL
#define tracePEND_FUNC_CALL(func, arg1, arg2, ret)                           \
  prvTraceStoreEvent1((ret == pdPASS) ? PSF_EVENT_TIMER_PENDFUNCCALL         \
                                      : PSF_EVENT_TIMER_PENDFUNCCALL_FAILED, \
                      (uint32_t)func);

#undef tracePEND_FUNC_CALL_FROM_ISR
#define tracePEND_FUNC_CALL_FROM_ISR(func, arg1, arg2, ret)              \
  prvTraceStoreEvent1((ret == pdPASS)                                    \
                          ? PSF_EVENT_TIMER_PENDFUNCCALL_FROMISR         \
                          : PSF_EVENT_TIMER_PENDFUNCCALL_FROMISR_FAILED, \
                      (uint32_t)func);
...
#define traceEVENT_GROUP_SYNC_BLOCK(eg, bitsToSet, bitsToWaitFor)        \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &          \
      CurrentFilterMask)                                                 \
    if (TRACE_GET_OBJECT_FILTER(EVENTGROUP, eg) & CurrentFilterMask)     \
      prvTraceStoreEvent2(PSF_EVENT_EVENTGROUP_SYNC_BLOCK, (uint32_t)eg, \
                          bitsToWaitFor);
...
#define traceEVENT_GROUP_SYNC_END(eg, bitsToSet, bitsToWaitFor, wasTimeout) \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &             \
      CurrentFilterMask)                                                    \
    if (TRACE_GET_OBJECT_FILTER(EVENTGROUP, eg) & CurrentFilterMask)        \
      prvTraceStoreEvent2((wasTimeout != pdTRUE)                            \
                              ? PSF_EVENT_EVENTGROUP_SYNC                   \
                              : PSF_EVENT_EVENTGROUP_SYNC_FAILED,           \
                          (uint32_t)eg, bitsToWaitFor);
...
#define traceEVENT_GROUP_WAIT_BITS_BLOCK(eg, bitsToWaitFor)                  \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &              \
      CurrentFilterMask)                                                     \
    if (TRACE_GET_OBJECT_FILTER(EVENTGROUP, eg) & CurrentFilterMask)         \
      prvTraceStoreEvent2(PSF_EVENT_EVENTGROUP_WAITBITS_BLOCK, (uint32_t)eg, \
                          bitsToWaitFor);
...
#define traceEVENT_GROUP_WAIT_BITS_END(eg, bitsToWaitFor, wasTimeout) \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &       \
    ...
    if (TRACE_GET_OBJECT_FILTER(EVENTGROUP, eg) & CurrentFilterMask)  \
      prvTraceStoreEvent2((wasTimeout != pdTRUE)                      \
                              ? PSF_EVENT_EVENTGROUP_WAITBITS         \
                              : PSF_EVENT_EVENTGROUP_WAITBITS_FAILED, \
                          (uint32_t)eg, bitsToWaitFor);
...
#define traceEVENT_GROUP_SET_BITS(eg, bitsToSet)                      \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &       \
      CurrentFilterMask)                                              \
    if (TRACE_GET_OBJECT_FILTER(EVENTGROUP, eg) & CurrentFilterMask)  \
      prvTraceStoreEvent2(PSF_EVENT_EVENTGROUP_SETBITS, (uint32_t)eg, \
                          bitsToSet);

#undef traceEVENT_GROUP_SET_BITS_FROM_ISR
#define traceEVENT_GROUP_SET_BITS_FROM_ISR(eg, bitsToSet)                   \
  if (TRACE_GET_OBJECT_FILTER(EVENTGROUP, eg) & CurrentFilterMask)          \
    prvTraceStoreEvent2(PSF_EVENT_EVENTGROUP_SETBITS_FROMISR, (uint32_t)eg, \
                        bitsToSet);
...
#define traceTASK_NOTIFY_TAKE(index)                                          \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &               \
      CurrentFilterMask) {                                                    \
    if (pxCurrentTCB->ucNotifyState[index] == taskNOTIFICATION_RECEIVED)      \
      prvTraceStoreEvent2(PSF_EVENT_TASK_NOTIFY_TAKE, (uint32_t)pxCurrentTCB, \
                          xTicksToWait);                                      \
    else                                                                      \
      prvTraceStoreEvent2(PSF_EVENT_TASK_NOTIFY_TAKE_FAILED,                  \
...
#define traceTASK_NOTIFY_TAKE_BLOCK(index)                      \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & \
      CurrentFilterMask)                                        \
    prvTraceStoreEvent2(PSF_EVENT_TASK_NOTIFY_TAKE_BLOCK,       \
                        (uint32_t)pxCurrentTCB, xTicksToWait);
#else /* TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_10_4_0 */
#define traceTASK_NOTIFY_TAKE_BLOCK()                           \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & \
...
#define traceTASK_NOTIFY_WAIT(index)                                          \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &               \
      CurrentFilterMask) {                                                    \
    if (pxCurrentTCB->ucNotifyState[index] == taskNOTIFICATION_RECEIVED)      \
      prvTraceStoreEvent2(PSF_EVENT_TASK_NOTIFY_WAIT, (uint32_t)pxCurrentTCB, \
                          xTicksToWait);                                      \
    else                                                                      \
      prvTraceStoreEvent2(PSF_EVENT_TASK_NOTIFY_WAIT_FAILED,                  \
                          (uint32_t)pxCurrentTCB, xTicksToWait);              \
  }
#elif (TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_9_0_0)
#define traceTASK_NOTIFY_WAIT()                                               \
...
#define traceTASK_NOTIFY_WAIT_BLOCK(index)                      \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & \
      CurrentFilterMask)                                        \
    prvTraceStoreEvent2(PSF_EVENT_TASK_NOTIFY_WAIT_BLOCK,       \
                        (uint32_t)pxCurrentTCB, xTicksToWait);
#else /* TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_10_4_0 */
#define traceTASK_NOTIFY_WAIT_BLOCK()                           \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & \
...
#define traceSTREAM_BUFFER_CREATE(pxStreamBuffer, xIsMessageBuffer)          \
  ...
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &              \
      CurrentFilterMask)                                                     \
    if (TRACE_GET_OBJECT_FILTER(STREAMBUFFER, pxStreamBuffer) &              \
        CurrentFilterMask)                                                   \
      prvTraceStoreEvent2(xIsMessageBuffer == 1                              \
                              ? PSF_EVENT_MESSAGEBUFFER_CREATE               \
                              : PSF_EVENT_STREAMBUFFER_CREATE,               \
                          (uint32_t)pxStreamBuffer, xBufferSizeBytes);
...
#define traceSTREAM_BUFFER_CREATE_FAILED(xIsMessageBuffer)          \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &     \
      CurrentFilterMask)                                            \
    prvTraceStoreEvent2(xIsMessageBuffer == 1                       \
                            ? PSF_EVENT_MESSAGEBUFFER_CREATE_FAILED \
                            : PSF_EVENT_STREAMBUFFER_CREATE_FAILED, \
                        0, xBufferSizeBytes);
...
#define traceSTREAM_BUFFER_DELETE(xStreamBuffer)                    \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &     \
    ...
    if (TRACE_GET_OBJECT_FILTER(STREAMBUFFER, pxStreamBuffer) &     \
        CurrentFilterMask)                                          \
      prvTraceStoreEvent2(prvGetStreamBufferType(xStreamBuffer) > 0 \
                              ? PSF_EVENT_MESSAGEBUFFER_DELETE      \
                              : PSF_EVENT_STREAMBUFFER_DELETE,      \
...
#define traceSTREAM_BUFFER_RESET(xStreamBuffer)                     \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &     \
      CurrentFilterMask)                                            \
    if (TRACE_GET_OBJECT_FILTER(STREAMBUFFER, xStreamBuffer) &      \
        CurrentFilterMask)                                          \
      prvTraceStoreEvent2(prvGetStreamBufferType(xStreamBuffer) > 0 \
                              ? PSF_EVENT_MESSAGEBUFFER_RESET       \
                              : PSF_EVENT_STREAMBUFFER_RESET,       \
                          (uint32_t)xStreamBuffer, 0);
...
#define traceSTREAM_BUFFER_SEND(xStreamBuffer, xReturn)             \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &     \
    ...
    if (TRACE_GET_OBJECT_FILTER(STREAMBUFFER, xStreamBuffer) &      \
        CurrentFilterMask)                                          \
      prvTraceStoreEvent2(prvGetStreamBufferType(xStreamBuffer) > 0 \
                              ? PSF_EVENT_MESSAGEBUFFER_SEND        \
                              : PSF_EVENT_STREAMBUFFER_SEND,        \
                          (uint32_t)xStreamBuffer,                  \
                          prvBytesInBuffer(xStreamBuffer));
...
#define traceBLOCKING_ON_STREAM_BUFFER_SEND(xStreamBuffer)          \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &     \
    ...
    if (TRACE_GET_OBJECT_FILTER(STREAMBUFFER, xStreamBuffer) &      \
        CurrentFilterMask)                                          \
      prvTraceStoreEvent1(prvGetStreamBufferType(xStreamBuffer) > 0 \
                              ? PSF_EVENT_MESSAGEBUFFER_SEND_BLOCK  \
                              : PSF_EVENT_STREAMBUFFER_SEND_BLOCK,  \
                          (uint32_t)xStreamBuffer);
...
#define traceSTREAM_BUFFER_SEND_FAILED(xStreamBuffer)               \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &     \
    ...
    if (TRACE_GET_OBJECT_FILTER(STREAMBUFFER, xStreamBuffer) &      \
        CurrentFilterMask)                                          \
      prvTraceStoreEvent1(prvGetStreamBufferType(xStreamBuffer) > 0 \
                              ? PSF_EVENT_MESSAGEBUFFER_SEND_FAILED \
                              : PSF_EVENT_STREAMBUFFER_SEND_FAILED, \
                          (uint32_t)xStreamBuffer);
...
#define traceSTREAM_BUFFER_RECEIVE(xStreamBuffer, xReceivedLength)  \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &     \
    ...
    if (TRACE_GET_OBJECT_FILTER(STREAMBUFFER, xStreamBuffer) &      \
        CurrentFilterMask)                                          \
      prvTraceStoreEvent2(prvGetStreamBufferType(xStreamBuffer) > 0 \
                              ? PSF_EVENT_MESSAGEBUFFER_RECEIVE     \
                              : PSF_EVENT_STREAMBUFFER_RECEIVE,     \
                          (uint32_t)xStreamBuffer,                  \
                          prvBytesInBuffer(xStreamBuffer));
...
#define traceBLOCKING_ON_STREAM_BUFFER_RECEIVE(xStreamBuffer)         \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &       \
    ...
    if (TRACE_GET_OBJECT_FILTER(STREAMBUFFER, xStreamBuffer) &        \
        CurrentFilterMask)                                            \
      prvTraceStoreEvent1(prvGetStreamBufferType(xStreamBuffer) > 0   \
                              ? PSF_EVENT_MESSAGEBUFFER_RECEIVE_BLOCK \
                              : PSF_EVENT_STREAMBUFFER_RECEIVE_BLOCK, \
                          (uint32_t)xStreamBuffer);
...
#define traceSTREAM_BUFFER_RECEIVE_FAILED(xStreamBuffer)               \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &        \
    ...
    if (TRACE_GET_OBJECT_FILTER(STREAMBUFFER, xStreamBuffer) &         \
        CurrentFilterMask)                                             \
      prvTraceStoreEvent1(prvGetStreamBufferType(xStreamBuffer) > 0    \
                              ? PSF_EVENT_MESSAGEBUFFER_RECEIVE_FAILED \
                              : PSF_EVENT_STREAMBUFFER_RECEIVE_FAILED, \
                          (uint32_t)xStreamBuffer);
...
#define traceSTREAM_BUFFER_SEND_FROM_ISR(xStreamBuffer, xReturn)             \
  if (TRACE_GET_OBJECT_FILTER(STREAMBUFFER, xStreamBuffer) &                 \
    ...
    if (xReturn > (size_t)0) {                                               \
      prvTraceStoreEvent2(prvGetStreamBufferType(xStreamBuffer) > 0          \
                              ? PSF_EVENT_MESSAGEBUFFER_SEND_FROM_ISR        \
                              : PSF_EVENT_STREAMBUFFER_SEND_FROM_ISR,        \
                          (uint32_t)xStreamBuffer,                           \
                          prvBytesInBuffer(xStreamBuffer));                  \
    } else {                                                                 \
      prvTraceStoreEvent1(prvGetStreamBufferType(xStreamBuffer) > 0          \
                              ? PSF_EVENT_MESSAGEBUFFER_SEND_FROM_ISR_FAILED \
                              : PSF_EVENT_STREAMBUFFER_SEND_FROM_ISR_FAILED, \
                          (uint32_t)xStreamBuffer);                          \
    }                                                                        \
  }
...
#define traceSTREAM_BUFFER_RECEIVE_FROM_ISR(xStreamBuffer, xReceivedLength) \
  if (TRACE_GET_OBJECT_FILTER(STREAMBUFFER, xStreamBuffer) &                \
    ...
    if (xReceivedLength > (size_t)0) {                                      \
      prvTraceStoreEvent2(prvGetStreamBufferType(xStreamBuffer) > 0         \
                              ? PSF_EVENT_MESSAGEBUFFER_RECEIVE_FROM_ISR    \
                              : PSF_EVENT_STREAMBUFFER_RECEIVE_FROM_ISR,    \
                          (uint32_t)xStreamBuffer,                          \
                          prvBytesInBuffer(xStreamBuffer));                 \
    } else {                                                                \
      prvTraceStoreEvent1(                                                  \
          prvGetStreamBufferType(xStreamBuffer) > 0                         \
              ? PSF_EVENT_MESSAGEBUFFER_RECEIVE_FROM_ISR_FAILED             \
