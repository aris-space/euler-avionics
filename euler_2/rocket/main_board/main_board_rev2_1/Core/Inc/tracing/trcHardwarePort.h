 ...
 *
 * TRC_HWTC_FREQ_HZ: The clock rate of the TRC_HWTC_COUNT counter in Hz. If
 *using TRC_OS_TIMER_INCR/DECR, this is should be TRC_HWTC_PERIOD *
 *TRACE_TICK_RATE_HZ. If using a free-running timer, this is often
 *TRACE_CPU_CLOCK_HZ (if running at the core clock rate). If using
 *TRC_CUSTOM_TIMER_INCR/DECR, this should match the clock rate of your custom
 *timer (i.e., TRC_HWTC_COUNT). If the default value of TRC_HWTC_FREQ_HZ is
 *incorrect for your setup, you can override it by calling vTraceSetFrequency
 *before calling vTraceEnable.
 *
 * TRC_HWTC_DIVISOR (used in snapshot mode only):
 * In snapshot mode, the timestamp resolution is
 *TRC_HWTC_FREQ_HZ/TRC_HWTC_DIVISOR. If the timer frequency is very high
 *(hundreds of MHz), we recommend increasing the TRC_HWTC_DIVISOR prescaler, to
 *reduce the bandwidth needed to store timestamps. This since extra "XTS" events
 *are inserted if the time since the previous event exceeds a certain limit (255
 *or 65535 depending on event type). It is advised to keep the time between most
 *events below 65535 native ticks (after division by TRC_HWTC_DIVISOR) to avoid
 *frequent XTS events.
 ******************************************************************************/
...
#ifndef __CORTEX_M
#error \
    "Can't find the CMSIS API. Please include your processor's header file in trcConfig.h"
#endif
 ...
 * If an AXI Timer 0 peripheral is available on your hardware platform, no
...
static inline uint32_t prvGetCPSR(void) {
  unsigned long ret;
  /* GCC-style assembly for getting the CPSR/APSR register, where the system
   * execution mode is found. */
 ...
 * If the name of the timer is sys_tmr SYSTEM_TIMER_BASE should be set to
 * SYS_TMR_BASE.
 */
...
static inline uint32_t altera_nios2_GetTimerSnapReg(void) {
  /* A processor can read the current counter value by first writing to either
   * snapl or snaph to request a coherent snapshot of the counter, and then
   * reading snapl and snaph for the full 32-bit value.
   */
  IOWR_ALTERA_AVALON_TIMER_SNAPL(SYSTEM_TIMER_BASE, 0);
  return (IORD_ALTERA_AVALON_TIMER_SNAPH(SYSTEM_TIMER_BASE) << 16) |
...
#define TRC_CA9_MPCORE_PRIVCTR_PERIOD_REG                          \
  (*(volatile uint32_t *)(TRC_CA9_MPCORE_PERIPHERAL_BASE_ADDRESS + \
                          TRC_CA9_MPCORE_PRIVATE_MEMORY_OFFSET + 0x00))
#define TRC_CA9_MPCORE_PRIVCTR_COUNTER_REG                         \
  (*(volatile uint32_t *)(TRC_CA9_MPCORE_PERIPHERAL_BASE_ADDRESS + \
                          TRC_CA9_MPCORE_PRIVATE_MEMORY_OFFSET + 0x04))
#define TRC_CA9_MPCORE_PRIVCTR_CONTROL_REG                         \
  (*(volatile uint32_t *)(TRC_CA9_MPCORE_PERIPHERAL_BASE_ADDRESS + \
...
#define TRC_CA9_MPCORE_PRIVCTR_PRESCALER               \
  (((TRC_CA9_MPCORE_PRIVCTR_CONTROL_REG &              \
     TRC_CA9_MPCORE_PRIVCTR_CONTROL_PRESCALER_MASK) >> \
...
core-clock usually), depending on the prescaler used. If a low prescaler is
used, the number of HW ticks between the trace events gets large, and thereby
inefficient to store (sometimes extra events are needed). To improve efficiency,
you may use the TRC_HWTC_DIVISOR as an additional prescaler.
*****************************************************************************************/
#define TRC_HWTC_DIVISOR 1
...
#define TRC_HWTC_COUNT              \
  PIT.TIMER[configTICK_PIT_CHANNEL] \
      .CVAL.R  // must be the PIT channel used for the systick
#define TRC_HWTC_PERIOD                        \
  ((configPIT_CLOCK_HZ / configTICK_RATE_HZ) - \
   1U)  // TODO FIXME or maybe not -1? what's the right "period" value?
#define TRC_HWTC_FREQ_HZ configPIT_CLOCK_HZ
#define TRC_HWTC_DIVISOR 1
#define TRC_IRQ_PRIORITY_ORDER \
  1  // higher IRQ priority values are more significant

#elif (TRC_CFG_HARDWARE_PORT == TRC_HARDWARE_PORT_APPLICATION_DEFINED)

#if !(defined(TRC_HWTC_TYPE) && defined(TRC_HWTC_COUNT) &&     \
      defined(TRC_HWTC_PERIOD) && defined(TRC_HWTC_FREQ_HZ) && \
      defined(TRC_IRQ_PRIORITY_ORDER))
