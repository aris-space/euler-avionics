...
void matmul(int32_t n, int32_t m, int32_t o, float A[n][m], float B[m][o], float C[n][o], bool reset);
...
void scalarmatprod(int32_t m, int32_t n, float scalar, float A[m][n], float B[m][n]);
...
void cofactor(int32_t dim, float A[dim][dim], float temp[dim][dim], int32_t p, int32_t q, int32_t n);
...
bool inverse(int32_t dim, float A[dim][dim], float inverse[dim][dim], float lambda);

/* Damped Moore-Penrose pseudo-inverse - ETHZ Robot Dynamics Lecture notes */
bool pseudo_inverse(int32_t m, int32_t n, float A[m][n], float inverse[n][m], float lambda);
...
int32_t polyfit(const float* const dependentValues, const float* const independentValues,
            uint32_t countOfElements, uint32_t order, double* coefficients);
