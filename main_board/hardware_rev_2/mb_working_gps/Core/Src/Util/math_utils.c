...
/* https://www.geeksforgeeks.org/adjoint-inverse-matrix/ */
void adjoint32_t(int dim, float A[dim][dim], float adj[dim][dim])
{
    if (dim == 1)
    {
        adj[0][0] = 1;
...
/* https://www.geeksforgeeks.org/adjoint-inverse-matrix/ */
bool inverse(int32_t dim, float A[dim][dim], float A_inv[dim][dim], float lambda)
{
...
bool pseudo_inverse(int32_t m, int32_t n, float A[m][n], float A_inv[n][m], float lambda) {
	float A_T[n][m];
	transpose(m, n, A, A_T);
